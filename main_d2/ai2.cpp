...
int player_is_visible_from_object(object *objp, vms_vector *pos, fix field_of_view, vms_vector *vec_to_player)
{
#ifdef USE_OX_BRIDGE
	{
		...
		// v2 packed format: header(8) + pv_ext(20) = 28 ints
		int packed_len = 8 + 20;
		int32_t packed[28];
		...
		packed[8 + 19] = objp->ctype.ai_info.SUB_FLAGS;

		int32_t out[11];
		cd_ox_player_is_visible_from_object_v2(packed, packed_len, out);

		int result = out[0];
		objp->ctype.ai_info.SUB_FLAGS = out[5];
		if (out[4])  // need_move_towards_center
		{
			pos->x = out[1]; pos->y = out[2]; pos->z = out[3];
			move_towards_segment_center(objp);
		}
		Hit_type = out[6];
		Hit_pos.x = out[7]; Hit_pos.y = out[8]; Hit_pos.z = out[9];
		Hit_seg = out[10];
...
void move_towards_vector(object *objp, vms_vector *vec_goal, int dot_based)
{
#ifdef USE_OX_BRIDGE
	static int ox_logged = 0;
	if (!ox_logged) { fprintf(stderr, "[OX] move_towards_vector (D2)\n"); ox_logged = 1; }
	...
	cd_ox_move_towards_vector(
		...
		dot_based, robptr->thief ? 1 : 0, robptr->kamikaze ? 1 : 0,
...
void move_around_player(object *objp, vms_vector *vec_to_player, int fast_flag)
{
#ifdef USE_OX_BRIDGE
	static int ox_logged = 0;
	if (!ox_logged) { fprintf(stderr, "[OX] move_around_player (D2)\n"); ox_logged = 1; }
	if (fast_flag == 0)
		return;
	physics_info *pptr = &objp->mtype.phys_info;
	robot_info *robptr = &Robot_info[objp->id];
	...
	int32_t packed[19] = {
		...
		player_cloaked, 1  /* skip_objnum1=true for D2 */
	};
	int32_t out_vx, out_vy, out_vz;
...
#ifdef USE_OX_BRIDGE
	static int ox_logged = 0;
	if (!ox_logged) { fprintf(stderr, "[OX] move_away_from_player (D2)\n"); ox_logged = 1; }
	physics_info *pptr = &objp->mtype.phys_info;
	robot_info *robptr = &Robot_info[objp->id];
	int objnum = objp - Objects;
	int32_t out_vx, out_vy, out_vz;
...
void ai_move_relative_to_player(object *objp, ai_local *ailp, fix dist_to_player, vms_vector *vec_to_player, fix circle_distance, int evade_only, int player_visibility)
{
#ifdef USE_OX_BRIDGE
	static int ox_logged = 0;
	if (!ox_logged) { fprintf(stderr, "[OX] ai_move_relative_to_player (D2)\n"); ox_logged = 1; }
	physics_info *pptr = &objp->mtype.phys_info;
	robot_info *robptr = &Robot_info[objp->id];
	...
	int32_t packed[56] = {
		...
		1,   /* is_d2 = true */                                           // 50
		player_visibility,                                                // 51
		(int32_t)((GameTime >> 18) & 0x0f),                               // 52
		robptr->companion ? 1 : 0,                                        // 53
		robptr->thief ? 1 : 0,                                            // 54
		robptr->kamikaze ? 1 : 0,                                         // 55
	};
	int32_t out[6];
...
void compute_vis_and_vec(object *objp, vms_vector *pos, ai_local *ailp, vms_vector *vec_to_player, int *player_visibility, robot_info *robptr, int *flag)
{
	...
	if (!*flag) {
		...
		// v2 packed format: header(8) + pv_ext(20) + cvv_ext(19) = 47 ints
		int packed_len = 8 + 20 + 19;
		int32_t packed[47];
		...
		packed[28 + 16] = 0;  // Player_exploded (D2 doesn't use it)
		packed[28 + 17] = ailp->next_fire2;
		packed[28 + 18] = ailp->player_awareness_type;
...
int do_robot_dying_frame(object *objp, fix start_time, fix roll_duration, int8_t *dying_sound_playing, int death_sound, fix expl_scale, fix sound_scale)
{
	fix	roll_val, temp;
	fix	sound_duration;
...
int do_any_robot_dying_frame(object *objp)
{
	if (objp->ctype.ai_info.dying_start_time) {
		...
		rval = do_robot_dying_frame(objp, objp->ctype.ai_info.dying_start_time, std::min(death_roll/2+1,6)*F1_0, &objp->ctype.ai_info.dying_sound_playing, Robot_info[objp->id].deathroll_sound, death_roll*F1_0/8, death_roll*F1_0/2);
