...
typedef struct netplayer_stats
{
	...
	unsigned int      flags;                                                  // Powerup flags, see below...
...
int message_length[MULTI_MAX_TYPE + 1] = {
   ...
   5,  // DOOR_OPEN
	...
	27, // ROBOT_POWERUPS
	7,  // HOSTAGE_DOOR
	2 + 24,   //SAVE_GAME             (uint8_t slot, unsigned int id, char name[20])
	2 + 4,   //RESTORE_GAME   (uint8_t slot, unsigned int id)
...
int find_goal_texture(uint8_t t);
...
multi_menu_poll(void)
{
	...
	I_Delay(100); // delay 100 milliseconds
...
multi_do_player_explode(char* buf)
{
	...
	count = 2;
	Players[pnum].primary_weapon_flags = (*(uint16_t*)(buf + count)); count += 2;
	Players[pnum].secondary_weapon_flags = (*(uint16_t*)(buf + count)); count += 2;
	...
	Players[pnum].primary_ammo[GAUSS_INDEX] = (*(uint16_t*)(buf + count)); count += 2;
	Players[pnum].flags = (*(unsigned int*)(buf + count));                     count += 4;
...
multi_do_controlcen_fire(char* buf)
{
	...
	to_target.x = (fix)((int)to_target.x);
	to_target.y = (fix)((int)to_target.y);
	to_target.z = (fix)((int)to_target.z);
#endif
...
void multi_do_drop_marker(char* buf)
{
	...
	position.x = (fix)(*(int*)(buf + 3));
	position.y = (fix)(*(int*)(buf + 7));
	position.z = (fix)(*(int*)(buf + 11));
...
void multi_do_save_game(char* buf)
{
	...
	id = (*(unsigned int*)(buf + count));              count += 4;
...
void multi_send_drop_marker(int player, vms_vector position, char messagenum, char text[])
{
	...
	if (player < N_players)
	{
		...
		*(fix*)(multibuf + 3) = (position.x);
		*(fix*)(multibuf + 7) = (position.y);
		*(fix*)(multibuf + 11) = (position.z);
		for (i = 0; i < 40; i++)
...
multi_send_player_explode(char type)
{
	...
	*(uint16_t*)(multibuf + count) = ((uint16_t)Players[Player_num].primary_weapon_flags);
	count += 2;
	*(uint16_t*)(multibuf + count) = ((uint16_t)Players[Player_num].secondary_weapon_flags);
	count += 2;
	...
	*(uint16_t*)(multibuf + count) = ((uint16_t)Players[Player_num].primary_ammo[VULCAN_INDEX]);
	count += 2;
	*(uint16_t*)(multibuf + count) = ((uint16_t)Players[Player_num].primary_ammo[GAUSS_INDEX]);
	count += 2;
	*(unsigned int*)(multibuf + count) = ((unsigned int)Players[Player_num].flags);
	count += 4;
...
extern uint8_t Secondary_weapon_to_powerup[];
extern uint8_t Primary_weapon_to_powerup[];
...
multi_send_kill(int objnum)
{
	...
	if (killer_objnum > -1) {
		short s;		// do it with variable since  won't work on return val from function.

		s = (short)objnum_local_to_remote(killer_objnum, (int8_t*)&multibuf[count + 2]);
		*(short*)(multibuf + count) = (s);
	}
	else
	{
		*(short*)(multibuf + count) = ((short)-1);
...
void
multi_send_door_open_specific(int pnum, int segnum, int side, uint8_t flag)
{
...
#else
	swapped_vec.x = (fix)((int)to_goal->x);
	swapped_vec.y = (fix)((int)to_goal->y);
	swapped_vec.z = (fix)((int)to_goal->z);
...
#else
	swapped_vec.x = (fix)((int)pos->x);
	swapped_vec.y = (fix)((int)pos->y);
	swapped_vec.z = (fix)((int)pos->z);
	memcpy(multibuf + count, &swapped_vec, 12);				count += 12;
#endif
	//                                                                                                            -----------
	//                                                                                                            Total =  19
...
void
multi_send_save_game(uint8_t slot, unsigned int id, char* desc)
{
	int count = 0;

	multibuf[count] = MULTI_SAVE_GAME;              count += 1;
	multibuf[count] = slot;                         count += 1;    // Save slot=0
	*(unsigned int*)(multibuf + count) = (id);         count += 4;             // Save id
...
void
multi_send_restore_game(uint8_t slot, unsigned int id)
{
	int count = 0;
...
multi_send_hostage_door_status(int wallnum)
{
	...
	*(short*)(multibuf + count) = ((short)wallnum);           count += 2;
	*(fix*)(multibuf + count) = (fix)((int)Walls[wallnum].hps);  count += 4;
...
void multi_initiate_save_game()
{
	...
	game_id ^= N_players << 4;
	for (i = 0; i < N_players; i++)
		game_id ^= *(unsigned int*)Players[i].callsign;
...
void multi_save_game(uint8_t slot, unsigned int id, char* desc)
{
	char filename[128];
...
void multi_restore_game(uint8_t slot, unsigned int id)
{
	char filename[128];
	player saved_player;
	int pnum, i;
...
void extract_netplayer_stats(netplayer_stats* ps, player* pd)
{
	...
	ps->energy = (fix)(pd->energy);                                                        // Amount of energy remaining.
	ps->shields = (fix)(pd->shields);                                                      // shields remaining (protection) 
	...
	for (i = 0; i < MAX_PRIMARY_WEAPONS; i++)
		ps->primary_ammo[i] = (pd->primary_ammo[i]);
	for (i = 0; i < MAX_SECONDARY_WEAPONS; i++)
		ps->secondary_ammo[i] = (pd->secondary_ammo[i]);
	...
	ps->cloak_time = (fix)(pd->cloak_time);							// Time cloaked
	ps->homing_object_dist = (fix)(pd->homing_object_dist);          //      Distance of nearest homing object.
	ps->invulnerable_time = (fix)(pd->invulnerable_time);			// Time invulnerable
	ps->KillGoalCount = (pd->KillGoalCount);
...
void use_netplayer_stats(player* ps, netplayer_stats* pd)
{
	...
	ps->flags = (pd->flags);							// Powerup flags, see below...
	ps->energy = (fix)((int)pd->energy);				// Amount of energy remaining.
	ps->shields = (fix)((int)pd->shields);			// shields remaining (protection) 
	ps->lives = pd->lives;									// Lives remaining, 0 = game over.
	ps->laser_level = pd->laser_level;						//	Current level of the laser.
	ps->primary_weapon_flags = pd->primary_weapon_flags;		//	bit set indicates the player has this weapon.
	ps->secondary_weapon_flags = pd->secondary_weapon_flags;	//	bit set indicates the player has this weapon.
	...
	ps->last_score = (pd->last_score);				// Score at beginning of current level.
	ps->score = (pd->score);							// Current score.
	ps->cloak_time = (fix)((int)pd->cloak_time);		// Time cloaked
	ps->homing_object_dist = (fix)((int)pd->homing_object_dist); //	Distance of nearest homing object.
	ps->invulnerable_time = (fix)((int)pd->invulnerable_time);	// Time invulnerable
	ps->KillGoalCount = (pd->KillGoalCount);
	ps->net_killed_total = (pd->net_killed_total);	// Number of times killed total
	ps->net_kills_total = (pd->net_kills_total);	// Number of net kills total
	ps->num_kills_level = (pd->num_kills_level);	// Number of kills this level
...
void multi_send_drop_weapon(int objnum, int seed)
{
	...
	*(short*)(multibuf + count) = (Player_num); count += 2;
	*(short*)(multibuf + count) = (objnum); count += 2;
	*(short*)(multibuf + count) = (ammo_count); count += 2;
	*(int*)(multibuf + count) = (seed);
...
void multi_send_wall_status_specific(int pnum, int wallnum, uint8_t type, uint8_t flags, uint8_t state)
{
...
void multi_send_heartbeat()
{
	...
	*(fix*)(multibuf + 1) = (fix)(ThisLevelTime);
...
void multi_send_seismic(fix start, fix end)
{
	...
	*(fix*)(multibuf + count) = (fix)(start); count += (sizeof(fix));
	*(fix*)(multibuf + count) = (fix)(end); count += (sizeof(fix));
...
void multi_do_seismic(char* buf)
{
	Seismic_disturbance_start_time = (fix)(*(int*)(buf + 1));
	Seismic_disturbance_end_time = (fix)(*(int*)(buf + 5));
	digi_play_sample(SOUND_SEISMIC_DISTURBANCE_START, F1_0);
}

void multi_send_light(int segnum, uint8_t val)
{
	int count = 1, i;
	multibuf[0] = MULTI_LIGHT;
	*(int*)(multibuf + count) = (segnum); count += (sizeof(int));
	*(char*)(multibuf + count) = val; count++;
	for (i = 0; i < 6; i++)
	{
		//mprintf ((0,"Sending %d!\n",Segments[segnum].sides[i].tmap_num2));
		*(short*)(multibuf + count) = (Segments[segnum].sides[i].tmap_num2); count += 2;
	}
	multi_send_data(multibuf, count, 1);
}
void multi_send_light_specific(int pnum, int segnum, uint8_t val)
{
	int count = 1, i;
...
void multi_do_light(char* buf)
{
	...
	for (i = 0; i < 6; i++)
	{
		if ((sides & (1 << i)))
		{
			...
			Segments[seg].sides[i].tmap_num2 = (*(short*)(buf + (6 + (2 * i))));
...
{
	char pnum = buf[1];
	unsigned int flags = (*(unsigned int*)(buf + 2));
...
void multi_send_flags(char pnum)
{
	...
	multibuf[1] = pnum;
	*(unsigned int*)(multibuf + 2) = (Players[pnum].flags);
...
#else
	* (int*)(multibuf + count) = (ActiveDoors[i].n_parts);					count += 4;
	*(short*)(multibuf + count) = (ActiveDoors[i].front_wallnum[0]);	count += 2;
	*(short*)(multibuf + count) = (ActiveDoors[i].front_wallnum[1]);	count += 2;
	*(short*)(multibuf + count) = (ActiveDoors[i].back_wallnum[0]);		count += 2;
	*(short*)(multibuf + count) = (ActiveDoors[i].back_wallnum[1]);		count += 2;
	*(int*)(multibuf + count) = (ActiveDoors[i].time);					count += 4;
#endif
...
#else
	ActiveDoors[i].n_parts = (*(int*)(buf + count));				count += 4;
	ActiveDoors[i].front_wallnum[0] = (*(short*)(buf + count));	count += 2;
	ActiveDoors[i].front_wallnum[1] = (*(short*)(buf + count));	count += 2;
	ActiveDoors[i].back_wallnum[0] = (*(short*)(buf + count));	count += 2;
	ActiveDoors[i].back_wallnum[1] = (*(short*)(buf + count));	count += 2;
	ActiveDoors[i].time = (*(int*)(buf + count));					count += 4;
#endif
}
...
#ifndef MACINTOSH
	* (unsigned int*)(multibuf + count) = sound;	count++;
#else
...
void multi_send_drop_flag(int objnum, int seed)
{
	...
	*(short*)(multibuf + count) = (Player_num); count += 2;
	*(short*)(multibuf + count) = (objnum); count += 2;
	*(short*)(multibuf + count) = (objp->ctype.powerup_info.count); count += 2;
	*(int*)(multibuf + count) = (seed);
...
void init_hoard_data()
{
	...
	for (i = 0; i < 2; i++)
	{
		...
		{
			uint8_t* bitmap_data;
			MALLOC(bitmap_data, uint8_t, icon_w * icon_h);
