...
void fuelcen_create(segment* segp)
{
...
#ifdef USE_OX_BRIDGE
	{
		...
		if (!registered) {
			cd_ox_register_fuelcen_create_effects(
				// fetch: [station_type, num_fuelcenters, max_fuelcens, fuelcen_max_amount,
				...
				[](int seg, int32_t* out) {
					...
					out[3] = Fuelcen_max_amount;
					out[4] = 1;  // is_d2
					out[5] = center.x;
					out[6] = center.y;
					out[7] = center.z;
				},
				// write: [segnum, station_type, max_capacity, timer, flag,
				//         center_x, center_y, center_z]
				[](const int32_t* packed, int len) {
					if (len < 8) return;
					int seg = packed[0];
					segment2* s2p = &Segment2s[seg];
					s2p->value = Num_fuelcenters;
...
#ifdef USE_OX_BRIDGE
	{
		...
		if (!registered) {
			cd_ox_register_fuelcen_activate_effects(
				// set_segment_special: sets Segment2s[segnum].special = station_type
				[](int seg, int special) {
					Segment2s[seg].special = special;
				});
			registered = true;
		}
		if (cd_ox_is_ready()) {
	...
	case 1:			// Wait until 1/2 second after VCLIP started.
		if (robotcen->Timer > (Vclip[VCLIP_MORPHING_ROBOT].play_time / 2)) {
			...
			if (RobotCenters[matcen_num].robot_flags[0] != 0 || RobotCenters[matcen_num].robot_flags[1] != 0) {
				int	type;
				uint32_t flags;
				int8_t legal_types[64];		//	64 bits, the width of robot_flags[].
