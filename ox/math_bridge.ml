let cd_vm_vec_add ax ay az bx by bz = Ox_math.vm_vec_add ~a:(ax, ay, az) ~b:(bx, by, bz)
let cd_vm_vec_sub ax ay az bx by bz = Ox_math.vm_vec_sub ~a:(ax, ay, az) ~b:(bx, by, bz)
let cd_vm_vec_add2 dx dy dz sx sy sz = Ox_math.vm_vec_add2 ~a:(dx, dy, dz) ~b:(sx, sy, sz)
let cd_vm_vec_sub2 dx dy dz sx sy sz = Ox_math.vm_vec_sub2 ~a:(dx, dy, dz) ~b:(sx, sy, sz)
let cd_vm_vec_avg ax ay az bx by bz = Ox_math.vm_vec_avg ~a:(ax, ay, az) ~b:(bx, by, bz)
