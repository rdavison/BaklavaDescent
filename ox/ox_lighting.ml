...
let compute_headlight_light_d1 ~point_x ~point_y ~point_z
...
let compute_headlight_light_d2 ~point_x ~point_y ~point_z
  ...
  let use_beam = has_headlight && headlight_on && (is_viewer <> 0)
                 && (player_energy > 0) in
  let light =
    if use_beam then beam_brightness * headlight_boost_scale
    else beam_brightness
  in
  if light = 0 then 0
  else begin
    let point_dist = Ox_math.vm_vec_mag_quick (point_x, point_y, point_z) in
