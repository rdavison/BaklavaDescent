...
let apply_damage_to_clutter ~flags ~shields ~damage ~obj_id =
  if flags land of_exploding <> 0 then (shields, 0)
  else if shields < 0 then (shields, 0)
  else begin
    let new_shields = shields - damage in
    if new_shields < 0 then begin
...
let apply_damage_to_controlcen
  ...
  if not who_is_player then shields
  else begin
    (* Network invulnerability timer *)
    if is_multiplayer && not is_coop && not time_level_ok then begin
      if who_is_local_player then
        Effect.perform Show_hud_invul_message;
      shields
    end else begin
      if who_is_local_player then
        Effect.perform Controlcen_been_hit;
      let new_shields =
        if shields >= 0 then shields - damage else shields
      in
      if new_shields < 0 && flags land (of_exploding lor of_destroyed) = 0 then begin
        Effect.perform (Do_controlcen_destroyed obj_id);
        if is_multiplayer then begin
          if who_objnum = local_player_objnum then
            Effect.perform Add_controlcen_score;
...
let apply_damage_to_player_d1
    ~is_dead ~is_invulnerable ~is_endlevel
    ~is_local_player
    ~player_shields ~damage ~killer_objnum =
  if is_dead then (player_shields, false)
  else if is_invulnerable then (player_shields, false)
  else if is_endlevel then (player_shields, false)
  else if not is_local_player then (player_shields, false)
  else begin
...
let apply_damage_to_player_d2
  ...
  if is_dead then (player_shields, false)
    ...
    if new_shields < 0 then begin
      Effect.perform (Set_player_dead killer_objnum);
      if killer_is_companion then
        Effect.perform Set_buddy_sorry_time;
      (new_shields, true)
    end else
      (new_shields, false)
  end
...
let maybe_kill_weapon_inner ~phys_flags ~weapon_shields ~other_type ~other_shields =
  if phys_flags land pf_persistent <> 0 || other_type = obj_weapon then begin
    (* Persistent weapons survive; non-persistent get weakened *)
    if phys_flags land pf_persistent <> 0 then
      (weapon_shields, false)
    else begin
      let dmg =
        if other_type = obj_weapon then other_shields / 2
        else other_shields / 4
      in
...
let maybe_kill_weapon_d1 ~weapon_id ~phys_flags ~weapon_shields
    ~other_type ~other_shields =
  if weapon_id = proximity_id then (weapon_shields, true)
  else maybe_kill_weapon_inner ~phys_flags ~weapon_shields ~other_type ~other_shields

let maybe_kill_weapon_d2 ~weapon_id ~phys_flags ~weapon_shields
    ~other_type ~other_shields ~is_shareware =
  if weapon_id = proximity_id || weapon_id = superprox_id || weapon_id = pmine_id then
    (weapon_shields, true)
  else if phys_flags land pf_persistent <> 0 || (is_shareware && other_type = obj_weapon) then begin
    if phys_flags land pf_persistent <> 0 then
      (weapon_shields, false)
    else begin
      let dmg =
...
let laser_are_related_d1 packed =
  let o1_idx = packed.(0) in let o1_type = packed.(1) in let o1_id = packed.(2) in
  let o1_sig = packed.(3) in let o1_pnum = packed.(4) in let o1_psig = packed.(5) in
  let o1_ctime = packed.(6) in
  let o2_idx = packed.(7) in let o2_type = packed.(8) in let o2_id = packed.(9) in
  let o2_sig = packed.(10) in let o2_pnum = packed.(11) in let o2_psig = packed.(12) in
  let _o2_ctime = packed.(13) in
  let game_time = packed.(14) in
  if o1_idx < 0 || o2_idx < 0 then 0
  (* See if o2 is the parent of o1 *)
  else if o1_type = obj_weapon
       && o1_pnum = o2_idx && o1_psig = o2_sig then
    if o1_id <> proximity_id || o1_ctime + f1_0 * 2 >= game_time then 1
    else 0
  (* See if o1 is the parent of o2 *)
  else if o2_type = obj_weapon
       && o2_pnum = o1_idx && o2_psig = o1_sig then
    1
  (* They must both be weapons *)
  else if o1_type <> obj_weapon || o2_type <> obj_weapon then 0
  (* Siblings with same parent *)
  else if o1_psig = o2_psig then
    if o1_id = proximity_id || o2_id = proximity_id then 0
    else 1
  else 0

let laser_are_related_d2 packed =
  let o1_idx = packed.(0) in let o1_type = packed.(1) in let o1_id = packed.(2) in
  let _o1_sig = packed.(3) in let o1_pnum = packed.(4) in let o1_psig = packed.(5) in
  let o1_ctime = packed.(6) in
  let o2_idx = packed.(7) in let o2_type = packed.(8) in let o2_id = packed.(9) in
  let _o2_sig = packed.(10) in let o2_pnum = packed.(11) in let o2_psig = packed.(12) in
  let o2_ctime = packed.(13) in
  let game_time = packed.(14) in
  if o1_idx < 0 || o2_idx < 0 then 0
  (* D2 time-based unrelate check for o1-is-weapon, o2-is-parent *)
  else if o1_type = obj_weapon
       && o1_pnum = o2_idx && o1_psig = packed.(10) (* o2_sig *) then
    if (o1_id = phoenix_id && game_time > o1_ctime + f1_0 / 4)
       || (o1_id = guidedmiss_id && game_time > o1_ctime + f1_0 * 2)
       || ((o1_id = proximity_id || o1_id = superprox_id) && game_time > o1_ctime + f1_0 * 4)
    then 0
    else 1
  (* D2 time-based unrelate check for o2-is-weapon, o1-is-parent *)
  else if o2_type = obj_weapon
       && o2_pnum = o1_idx && o2_psig = packed.(3) (* o1_sig *) then
    if (o2_id = phoenix_id && game_time > o2_ctime + f1_0 / 4)
       || (o2_id = guidedmiss_id && game_time > o2_ctime + f1_0 * 2)
       || ((o2_id = proximity_id || o2_id = superprox_id) && game_time > o2_ctime + f1_0 * 4)
    then 0
    else 1
  (* Both must be weapons *)
  else if o1_type <> obj_weapon || o2_type <> obj_weapon then 0
  (* Siblings with same parent — prox/superprox time check *)
  else if o1_psig = o2_psig then
    if o1_id = proximity_id || o2_id = proximity_id
       || o1_id = superprox_id || o2_id = superprox_id then
      if game_time > o1_ctime + f1_0 / 2 || game_time > o2_ctime + f1_0 / 2 then 0
      else 1
    else 1
  (* Mine/prox/robot-superprox: never related *)
  else if o1_id = robot_superprox_id || o2_id = robot_superprox_id
       || o1_id = proximity_id || o2_id = proximity_id
       || o1_id = superprox_id || o2_id = superprox_id
       || o1_id = pmine_id || o2_id = pmine_id then 0
  else 1
...
let chase_angles ~cur_p ~cur_b ~cur_h ~desired_p ~desired_b ~desired_h ~frame_time =
  ...
  let cur_p, cur_b, cur_h, dp, db, dh =
    if alt_total_delta < total_delta then
      (alt_p, alt_b, alt_h, alt_dp, alt_db, alt_dh)
    else
  ...
  let new_p =
    ...
    else if dp > 0 then cur_p + frame_turn
    else cur_p - frame_turn
  in
  let new_b =
    if abs db < frame_turn then begin mask := !mask lor 2; desired_b end
    else if db > 0 then cur_b + frame_turn
    else cur_b - frame_turn
  in
  let new_h =
    if abs dh < frame_turn then begin mask := !mask lor 4; desired_h end
    else if dh > 0 then cur_h + frame_turn
...
let calc_best_gun packed =
  ...
  let ox = packed.(obj_off) and oy = packed.(obj_off + 1) and oz = packed.(obj_off + 2) in
...
let apply_damage_to_robot_d2
  ...
  (* Robot already dead? *)
    ...
    if is_companion && is_final_level then
      (shields, 0)
    else begin
      let new_shields = ref (shields - damage) in
      (* Final boss hacks: prevent boss death in certain conditions *)
      if is_boss && is_final_level && !new_shields < 0 then begin
        if is_multiplayer then begin
          if not (Effect.perform Query_multi_all_players_alive) then
            new_shields := 1
          else begin
          ...
          if Effect.perform Query_player_dead_or_no_shields then
            new_shields := 1
          else
            Effect.perform Do_final_boss_hacks
        end
      ...
      if !new_shields < 0 then begin
        if is_multiplayer then begin
          ...
          if Effect.perform (Query_multi_explode_d2 (obj_id, killer_objnum, is_thief)) then begin
          ...
          Effect.perform Increment_kills;
          if is_boss then
            Effect.perform (Start_boss_death obj_id)
          else if is_death_roll then
            Effect.perform (Start_robot_death_sequence obj_id)
          else begin
            if robot_id = special_reactor_robot then
              Effect.perform Special_reactor_stuff;
            if is_kamikaze then
              Effect.perform (Explode_object_delay (obj_id, 1))
            else
              Effect.perform (Explode_object_delay (obj_id, standard_expl_delay_d2))
          end;
          (!new_shields, 1)
        end
      end else
