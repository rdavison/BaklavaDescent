...
let check_unop_i64 name c_impl ox_impl cases =
  List.iter cases ~f:(fun q ->
    let c = c_impl q in
    let ox = ox_impl q in
    printf "%s q=%s c=%d ox=%d eq=%b\n" name (Int64.to_string q) c ox (Int.equal c ox))
;;
...
external c_vm_vec_scale2
  ...
  -> int
  -> int * int * int
  = "caml_c_vm_vec_scale2"

external c_vm_vec_add
  :  int
  -> int
  -> int
  -> int
...
external c_vm_vec_copy_scale
  :  int
  -> int
  -> int
  -> int
  -> int * int * int
  = "caml_c_vm_vec_copy_scale"

external c_vm_vec_scale
  :  int
  -> int
  -> int
  -> int
...
external c_vm_vec_copy_normalize
  :  int
  -> int
  -> int
  -> int * int * int * int
...
external c_vm_vec_normalize
  :  int
  -> int
  -> int
  -> int * int * int * int
  = "caml_c_vm_vec_normalize"

external c_vm_vec_normalized_dir
  :  int
  -> int
  -> int
  -> int
...
external c_vm_vec_copy_normalize_quick
  :  int
  -> int
  -> int
  -> int * int * int * int
...
external c_vm_vec_normalize_quick
  :  int
  -> int
  -> int
  -> int * int * int * int
...
external c_vm_vec_normalized_dir_quick
  :  int
  -> int
  -> int
  -> int
...
let eq_vec3 (ax, ay, az) (bx, by, bz) =
  Int.equal ax bx && Int.equal ay by && Int.equal az bz
;;

let eq_mat3 (ar, au, af) (br, bu, bf) = eq_vec3 ar br && eq_vec3 au bu && eq_vec3 af bf

let flat_of_mat3 (((r1, r2, r3), (u1, u2, u3), (f1, f2, f3)) : mat3) =
  r1, r2, r3, u1, u2, u3, f1, f2, f3
;;

let mat3_of_flat (r1, r2, r3, u1, u2, u3, f1, f2, f3) : mat3 =
  (r1, r2, r3), (u1, u2, u3), (f1, f2, f3)
;;
...
let c_vm_vec_ang_2_matrix (vx, vy, vz) a =
  mat3_of_flat (c_vm_vec_ang_2_matrix_raw vx vy vz a)
;;
...
let vm_vec_perp_cases =
  ...
  ; (12345, -54321, 99999), (67890, 13579, -24680), (-11111, 22222, -33333)
  ; ( (Int32.to_int_exn Int32.max_value, 0, Int32.to_int_exn (Int32.succ Int32.min_value))
    , (1, -1, 2)
    , (7, -7, 14) )
  ]
;;
...
let vm_vec_delta_ang_cases =
  ...
  ; ( (Int32.to_int_exn Int32.max_value, 1, Int32.to_int_exn Int32.min_value)
    , (1, -1, 2)
    , None )
  ]
;;
...
let mat_sample = (12345, -54321, 99999), (67890, 13579, -24680), (-11111, 22222, -33333)
...
let vm_extract_angles_vector_cases =
  ...
  ; ( (Int32.to_int_exn Int32.max_value, 17, Int32.to_int_exn Int32.min_value)
    , (12345, -54321, 99999) )
  ]
;;
...
let vm_extract_angles_matrix_cases =
  ...
  ; mat_sample
  ; mat_extreme
  ; Ox_math.vm_angles_2_matrix ~v:(0x1000, 0x2000, 0x3000)
  ]
;;
...
let fix_nonneg_gen =
  Quickcheck.Generator.weighted_union
    ...
    ; ( 7.0
      , Quickcheck.Generator.map Int32.quickcheck_generator ~f:(fun v ->
          Int.max 0 (Int32.to_int_exn v)) )
    ]
;;
...
let int64_edge_values =
  ...
  ; 0L
  ; 1L
  ; Int64.of_int Int.min_value
  ; Int64.of_int Int.max_value
  ]
;;
    ...
    ; ( 7.0
      , Quickcheck.Generator.map
          (Quickcheck.Generator.both
             ...
             Int32.quickcheck_generator)
          ~f:(fun (hi, lo) -> from_halves hi lo) )
    ]
;;

let clamp_fixmulaccum_q q =
  Int.max (-9_000_000_000_000_000) (Int.min 9_000_000_000_000_000 q)
;;

let fixmulaccum_q_gen =
  Quickcheck.Generator.map Int.quickcheck_generator ~f:(fun q ->
    Int64.of_int (clamp_fixmulaccum_q q))
;;

let small_fix_gen =
  Quickcheck.Generator.map Int.quickcheck_generator ~f:(fun q ->
    Int.max (-1_000_000) (Int.min 1_000_000 q))
;;

let random_values ~seed ~test_count gen =
  Quickcheck.random_sequence ~seed:(`Deterministic seed) gen
  |> fun seq -> Sequence.take seq test_count
;;
...
let vec3_option_gen =
  Quickcheck.Generator.weighted_union
    ...
    ; 3.0, Quickcheck.Generator.map vec3_gen ~f:Option.some
    ]
;;
  ...
  Quickcheck.Generator.weighted_union
    [ ( 3.0
      , Quickcheck.Generator.of_list
          [ -0x3fffffff; -0x10000; -1; 0; 1; 0x10000; 0x3fffffff ] )
    ; ( 7.0
      , Quickcheck.Generator.map Int32.quickcheck_generator ~f:(fun v ->
          clamp_mag_safe (Int32.to_int_exn v)) )
    ]
;;
...
let vec3_mag_safe_gen =
  Quickcheck.Generator.map
    (Quickcheck.Generator.both
       fix_mag_safe_gen
       (Quickcheck.Generator.both fix_mag_safe_gen fix_mag_safe_gen))
    ~f:(fun (x, (y, z)) -> x, y, z)
;;
...
let run_random_unop_i64 ~name ~seed ~test_count c_impl ox_impl =
  ...
  |> Sequence.iter ~f:(fun q ->
    ...
    then (
      ...
      then
        first_mismatch
        := Some (sprintf "%s q=%s c=%d ox=%d" name (Int64.to_string q) c ox)));
  printf "%s random total=%d mismatches=%d\n" name !total !mismatches;
  Option.iter !first_mismatch ~f:(fun s -> printf "first_mismatch %s\n" s);
  if !mismatches <> 0 then failwithf "%s randomized parity failed" name ()
;;
...
let run_random_binop_i64_int_with_gen ~name ~seed ~test_count ~gen c_impl ox_impl =
  ...
  |> Sequence.iter ~f:(fun (n, d) ->
    ...
    then (
      ...
      then
        first_mismatch
        := Some (sprintf "%s n=%s d=%d c=%d ox=%d" name (Int64.to_string n) d c ox)));
  printf "%s random total=%d mismatches=%d\n" name !total !mismatches;
  Option.iter !first_mismatch ~f:(fun s -> printf "first_mismatch %s\n" s);
  if !mismatches <> 0 then failwithf "%s randomized parity failed" name ()
;;
...
let run_random_unop_pair ~name ~seed ~test_count c_impl ox_impl =
  ...
  |> Sequence.iter ~f:(fun a ->
    ...
    then (
      ...
      then
        first_mismatch
        := Some (sprintf "%s a=%d c=(%d,%d) ox=(%d,%d)" name a c_s c_c ox_s ox_c)));
  printf "%s random total=%d mismatches=%d\n" name !total !mismatches;
  Option.iter !first_mismatch ~f:(fun s -> printf "first_mismatch %s\n" s);
  if !mismatches <> 0 then failwithf "%s randomized parity failed" name ()
;;
...
let run_random_binop_pair ~name ~seed ~test_count c_impl ox_impl =
  ...
  |> Sequence.iter ~f:(fun (a, b) ->
    ...
    then (
      ...
      then
        first_mismatch
        := Some (sprintf "%s a=%d b=%d c=(%d,%d) ox=(%d,%d)" name a b c_x c_y ox_x ox_y)));
  printf "%s random total=%d mismatches=%d\n" name !total !mismatches;
  Option.iter !first_mismatch ~f:(fun s -> printf "first_mismatch %s\n" s);
  if !mismatches <> 0 then failwithf "%s randomized parity failed" name ()
;;
...
let check_vec3_avg4 name c_impl ox_impl cases =
  List.iter cases ~f:(fun ((a1, a2, a3), (b1, b2, b3), (c1, c2, c3), (d1, d2, d3)) ->
    ...
    printf
      "%s a=(%d,%d,%d) b=(%d,%d,%d) c=(%d,%d,%d) d=(%d,%d,%d) out=(%d,%d,%d) \
       ox=(%d,%d,%d) eq=%b\n"
      name
      a1
      a2
...
let run_random_two_vec3_opt_to_scalar ~name ~seed ~test_count c_impl ox_impl =
  let gen =
    Quickcheck.Generator.map
      (Quickcheck.Generator.both
         vec3_gen
         (Quickcheck.Generator.both vec3_gen vec3_option_gen))
  ...
  |> Sequence.iter ~f:(fun ((ex, ey, ez), (sx, sy, sz)) ->
    ...
    then (
      ...
      then
        ...
        := Some
             (sprintf
                "%s end=(%d,%d,%d) start=(%d,%d,%d) m=(c:%d ox:%d) out=(c:%d,%d,%d \
                 ox:%d,%d,%d)"
                name
                ex
                ey
...
let vec3_no_min_gen =
  Quickcheck.Generator.map
    (Quickcheck.Generator.both
       fix_no_min_gen
       (Quickcheck.Generator.both fix_no_min_gen fix_no_min_gen))
    ~f:(fun (x, (y, z)) -> x, y, z)
;;
...
let run_random_three_vec3_to_vec3_no_min ~name ~seed ~test_count c_impl ox_impl =
  let gen =
    Quickcheck.Generator.map
      (Quickcheck.Generator.both
         vec3_perp_gen
         (Quickcheck.Generator.both vec3_perp_gen vec3_perp_gen))
      ~f:(fun (a, (b, c)) -> a, b, c)
  in
  let total = ref 0 in
  let mismatches = ref 0 in
...
let check_mat_unop
  ...
  List.iter cases ~f:(fun m ->
    ...
    let ox_r1, ox_r2, ox_r3, ox_u1, ox_u2, ox_u3, ox_f1, ox_f2, ox_f3 =
      flat_of_mat3 ox_m
    in
    printf
...
let check_vec3_opt_opt_to_mat
  ...
  List.iter cases ~f:(fun (fvec, uvec, rvec) ->
    ...
    let fx, fy, fz = fvec in
    printf
      "%s f=(%d,%d,%d) u=%s r=%s c=[%d,%d,%d;%d,%d,%d;%d,%d,%d] \
       ox=[%d,%d,%d;%d,%d,%d;%d,%d,%d] eq=%s\n"
      name
      fx
      fy
      fz
...
let check_six_fix_to_mat
  ...
  List.iter cases ~f:(fun (sinp, cosp, sinb, cosb, sinh, cosh) ->
    ...
    in
    printf
      "%s s=(%d,%d,%d,%d,%d,%d) c=[%d,%d,%d;%d,%d,%d;%d,%d,%d] \
       ox=[%d,%d,%d;%d,%d,%d;%d,%d,%d] eq=%s\n"
      name
      sinp
      cosp
...
let check_ang3_to_mat
  ...
  List.iter cases ~f:(fun ((p, b, h) as ang) ->
    ...
    in
    printf
      "%s a=(%d,%d,%d) c=[%d,%d,%d;%d,%d,%d;%d,%d,%d] ox=[%d,%d,%d;%d,%d,%d;%d,%d,%d] \
       eq=%s\n"
      name
      p
      b
...
let check_vec3_fixang_to_mat
  ...
  List.iter cases ~f:(fun (v, a) ->
    ...
    in
    printf
      "%s v=(%d,%d,%d) a=%d c=[%d,%d,%d;%d,%d,%d;%d,%d,%d] \
       ox=[%d,%d,%d;%d,%d,%d;%d,%d,%d] eq=%s\n"
      name
      vx
      vy
...
let ang3_gen =
  Quickcheck.Generator.map
    (Quickcheck.Generator.both
       fixang_gen
       (Quickcheck.Generator.both fixang_gen fixang_gen))
    ~f:(fun (x, (y, z)) -> x, y, z)
;;
...
let%expect_test "fix_atan2 parity C vs Ox" =
  check_binop
    "fix_atan2"
    c_fix_atan2
    (fun cos sin -> Ox_math.fix_atan2 ~cos ~sin)
    fix_atan2_cases;
  [%expect
    {|
...
let%expect_test "vm_vec_scale_add parity C vs Ox" =
  check_vec3_bin_scale
    "vm_vec_scale_add"
    c_vm_vec_scale_add
    (fun a b k -> Ox_math.vm_vec_scale_add ~a ~b ~k)
    vm_vec_scale_add_cases;
  [%expect
    {|
...
let%expect_test "sincos_2_matrix parity C vs Ox" =
  check_six_fix_to_mat
    "sincos_2_matrix"
    c_sincos_2_matrix
    (fun sinp cosp sinb cosb sinh cosh ->
...
let%expect_test "randomized fixmulaccum parity C vs Ox" =
  let gen =
    Quickcheck.Generator.map
      (Quickcheck.Generator.both
         fixmulaccum_q_gen
         (Quickcheck.Generator.both small_fix_gen small_fix_gen))
      ~f:(fun (q, (a, b)) -> q, a, b)
  in
...
let%expect_test "randomized fix_sqrt parity C vs Ox" =
  run_random_unop
    ~name:"fix_sqrt"
    ~seed:"fix-sqrt-seed-v1"
    ~test_count:5000
    c_fix_sqrt
    (fun a -> Ox_math.fix_sqrt ~a);
...
let%expect_test "randomized fix_acos parity C vs Ox" =
  run_random_unop
    ~name:"fix_acos"
    ~seed:"fix-acos-seed-v1"
    ~test_count:5000
    c_fix_acos
    (fun a -> Ox_math.fix_acos ~a);
...
let%expect_test "randomized fix_asin parity C vs Ox" =
  run_random_unop
    ~name:"fix_asin"
    ~seed:"fix-asin-seed-v1"
    ~test_count:5000
    c_fix_asin
    (fun a -> Ox_math.fix_asin ~a);
...
let%expect_test "g3_code_point parity C vs Ox" =
  List.iter g3_code_point_cases ~f:(fun (x, y, z) ->
    ...
    printf
      "g3_code_point (%d,%d,%d) c=0x%02x ox=0x%02x eq=%b\n"
      x
      y
      z
      c
...
let%expect_test "checkmuldiv parity C vs Ox" =
  List.iter checkmuldiv_cases ~f:(fun (a, b, c) ->
    ...
    printf
      ...
      ox_ok_i
      ox_r
      (Int.equal c_ok ox_ok_i && Int.equal c_r ox_r));
  [%expect
    {|
...
let%expect_test "g3_rotate_point parity C vs Ox" =
  ...
  List.iter
    cases
    ~f:(fun ((sx, sy, sz), (vpx, vpy, vpz), (r1, r2, r3, u1, u2, u3, f1, f2, f3)) ->
      ...
      printf
        "g3_rotate_point c=(%d,%d,%d,0x%02x) ox=(%d,%d,%d,0x%02x) eq=%b\n"
        c_rx
        c_ry
        c_rz
        c_codes
...
let%expect_test "g3_project_point parity C vs Ox" =
  ...
  List.iter cases ~f:(fun (x, y, z, cw2, ch2) ->
    ...
    printf
      "g3_project_point (%d,%d,%d) c_ok=%d c=(%d,%d) ox_ok=%d ox=(%d,%d) eq=%b\n"
      x
      y
      z
      c_ok
...
let%expect_test "g3_rotate_delta_x parity C vs Ox" =
  ...
  printf
    "g3_rotate_delta_x c=(%d,%d,%d) ox=(%d,%d,%d) eq=%b\n"
    c_rx
    c_ry
    c_rz
    ox_rx
...
let%expect_test "g3_rotate_delta_y parity C vs Ox" =
  ...
  let eq = c_rx = ox_rx && c_ry = ox_ry && c_rz = ox_rz in
  printf
    "g3_rotate_delta_y c=(%d,%d,%d) ox=(%d,%d,%d) eq=%b\n"
    c_rx
    c_ry
    c_rz
    ox_rx
...
let%expect_test "g3_rotate_delta_z parity C vs Ox" =
  ...
  let eq = c_rx = ox_rx && c_ry = ox_ry && c_rz = ox_rz in
  printf
    "g3_rotate_delta_z c=(%d,%d,%d) ox=(%d,%d,%d) eq=%b\n"
    c_rx
    c_ry
    c_rz
    ox_rx
...
let%expect_test "g3_calc_point_depth parity C vs Ox" =
  ...
  List.iter cases ~f:(fun ((px, py, pz), (vpx, vpy, vpz), (fx, fy, fz)) ->
    ...
    in
    printf
      "g3_calc_point_depth c=%d ox=%d eq=%b\n"
      c_depth
      ox_depth
      (Int.equal c_depth ox_depth));
  [%expect
    {|
...
let%expect_test "scale_matrix parity C vs Ox" =
  let cases =
    [ (* identity matrix, unity window scale, zoom = 1.0 *)
      (0x10000, 0, 0, 0, 0x10000, 0, 0, 0, 0x10000), (0x10000, 0x10000, 0x10000), 0x10000
    ; (* zoom in (zoom < 1.0) *)
      (0x10000, 0, 0, 0, 0x10000, 0, 0, 0, 0x10000), (0x10000, 0x10000, 0x10000), 0x8000
    ; (* zoom out (zoom > 1.0) *)
      (0x10000, 0, 0, 0, 0x10000, 0, 0, 0, 0x10000), (0x10000, 0x10000, 0x10000), 0x20000
    ]
  in
  List.iter cases ~f:(fun ((r1, r2, r3, u1, u2, u3, f1, f2, f3), (wsx, wsy, wsz), zoom) ->
    ...
    let scaled_mat, (omsx, omsy, omsz), _unscaled =
      Ox_3d.scale_matrix ((r1, r2, r3), (u1, u2, u3), (f1, f2, f3)) (wsx, wsy, wsz) zoom
    in
    ...
    in
    printf
      "scale_matrix zoom=%d c_mat=(%d,%d,%d/%d,%d,%d/%d,%d,%d) c_ms=(%d,%d,%d) eq=%b\n"
      zoom
      cr1
      cr2
      cr3
...
let%expect_test "randomized g3_rotate_point parity C vs Ox" =
  ...
  |> Sequence.iter
       ~f:
         (fun
           ((sx, sy, sz), (vpx, vpy, vpz), ((r1, r2, r3), (u1, u2, u3), (f1, f2, f3))) ->
         incr total;
         let c_rx, c_ry, c_rz, c_codes =
           c_g3_rotate_point sx sy sz vpx vpy vpz r1 r2 r3 u1 u2 u3 f1 f2 f3
         in
...
let%expect_test "randomized g3_project_point parity C vs Ox" =
  ...
  let gen =
    Quickcheck.Generator.map
      (Quickcheck.Generator.both
         vec3_gen
         (Quickcheck.Generator.both pos_fix_gen pos_fix_gen))
  ...
  |> Sequence.iter ~f:(fun (((r1, r2, r3), (u1, u2, u3), (f1, f2, f3)), dx) ->
    ...
    then (
      ...
      then
        ...
        := Some
             (sprintf
                "g3_rotate_delta_x c=(%d,%d,%d) ox=(%d,%d,%d)"
                c_rx
                c_ry
                c_rz
                ox_rx
  ...
  |> Sequence.iter ~f:(fun (((r1, r2, r3), (u1, u2, u3), (f1, f2, f3)), dy) ->
    ...
    then (
      ...
      then
        ...
        := Some
             (sprintf
                "g3_rotate_delta_y c=(%d,%d,%d) ox=(%d,%d,%d)"
                c_rx
                c_ry
                c_rz
                ox_rx
  ...
  |> Sequence.iter ~f:(fun (((r1, r2, r3), (u1, u2, u3), (f1, f2, f3)), dz) ->
    ...
    then (
      ...
      then
        ...
        := Some
             (sprintf
                "g3_rotate_delta_z c=(%d,%d,%d) ox=(%d,%d,%d)"
                c_rx
                c_ry
                c_rz
                ox_rx
...
let%expect_test "randomized g3_calc_point_depth parity C vs Ox" =
  ...
  |> Sequence.iter ~f:(fun ((px, py, pz), (vpx, vpy, vpz), (fx, fy, fz)) ->
    ...
    then (
      incr mismatches;
      if Option.is_none !first_mismatch
      then first_mismatch := Some (sprintf "g3_calc_point_depth c=%d ox=%d" c_d ox_d)));
...
let%expect_test "randomized scale_matrix parity C vs Ox" =
  ...
  |> Sequence.iter
       ~f:(fun (((r1, r2, r3), (u1, u2, u3), (f1, f2, f3)), (wsx, wsy, wsz), zoom) ->
         incr total;
         let cr1, cr2, cr3, cu1, cu2, cu3, cf1, cf2, cf3, cmsx, cmsy, cmsz =
           c_scale_matrix r1 r2 r3 u1 u2 u3 f1 f2 f3 wsx wsy wsz zoom
         in
         ...
         then (
           ...
           then
             ...
             := Some
                  (sprintf
                     "scale_matrix zoom=%d c_ms=(%d,%d,%d) ox_ms=(%d,%d,%d)"
                     zoom
                     cmsx
                     cmsy
                     cmsz
...
let%expect_test "randomized g3_start_instance_matrix parity C vs Ox" =
  let gen =
    Quickcheck.Generator.map
      (Quickcheck.Generator.both
         ...
         (Quickcheck.Generator.both
            vec3_gen
            (Quickcheck.Generator.both (Quickcheck.Generator.of_list [ 0; 1 ]) mat3_gen)))
       ...
       ~f:
         ...
         then (
           ...
           then
             ...
             := Some
                  (sprintf
                     "g3_start_instance_matrix has_orient=%d c_vp=(%d,%d,%d) \
                      ox_vp=(%d,%d,%d)"
                     has_orient
                     cnvpx
                     cnvpy
  ...
  Option.iter !first_mismatch ~f:(fun s -> printf "first_mismatch %s\n" s);
  if !mismatches <> 0
  then failwithf "g3_start_instance_matrix randomized parity failed" ();
  [%expect {| g3_start_instance_matrix random total=5000 mismatches=0 |}]
;;

let%expect_test "randomized g3_point_2_vec parity C vs Ox" =
  let short_gen =
    Quickcheck.Generator.map Int.quickcheck_generator ~f:(fun v ->
      Int.max (-32768) (Int.min 32767 v))
  in
  let nonzero_fix_gen =
    Quickcheck.Generator.map fix_gen ~f:(fun v ->
      let v = Ox_math.wrap_i64_to_fix (Int64.of_int v) in
         ...
         then (
           ...
           then (
             ...
             then
               ...
               := Some
                    (sprintf
                       "g3_point_2_vec (%d,%d) c=(%d,%d,%d) ox=(%d,%d,%d)"
                       sx
                       sy
                       cvx
                       cvy
       ...
       ~f:
         ...
         then (
           ...
           then
             ...
             := Some
                  (sprintf
                     "clip_edge pf=%d c=(%d,%d,%d) ox=(%d,%d,%d) c_codes=%d ox_codes=%d"
                     plane_flag
                     cnx
                     cny
                     cnz
...
let%expect_test "randomized clip_line parity C vs Ox" =
  let codes_gen =
    Quickcheck.Generator.map
      ...
      ~f:(fun ((x, y, z), _) ->
        ...
        x, y, z, cc)
  in
  let gen = Quickcheck.Generator.both codes_gen codes_gen in
  let total = ref 0 in
  let mismatches = ref 0 in
  let first_mismatch = ref None in
  ...
  |> Sequence.iter ~f:(fun ((p0x, p0y, p0z, p0c), (p1x, p1y, p1z, p1c)) ->
    ...
    if codes_or land 0x80 = 0 && codes_or <> 0
    then (
      incr total;
      ...
      then (
        ...
        then
          ...
          := Some
               (sprintf
                  "clip_line c_p0=(%d,%d,%d/%d) o_p0=(%d,%d,%d/%d) c_clip=%d o_clip=%d"
                  cp0x
                  cp0y
                  cp0z
                  cp0c
...
external c_clip_polygon : int -> int -> int array -> int array = "caml_c_clip_polygon"
...
let%expect_test "randomized clip_polygon parity C vs Ox" =
  ...
  |> Sequence.iter ~f:(fun (nv, verts) ->
    ...
    let codes_list =
      List.map verts ~f:(fun ((x, y, z, _u, _v), _flags) -> Ox_3d.g3_code_point (x, y, z))
    in
    ...
    if codes_or land 0x80 = 0 && codes_or <> 0 && codes_and = 0
    then (
      incr total;
      ...
      (* OCaml *)
      let points =
        List.mapi verts ~f:(fun i ((x, y, z, u, v), flags) ->
          let codes = List.nth_exn codes_list i in
          x, y, z, u, v, 0, flags, codes)
      in
      ...
      then (
        ...
        then (
          ...
          := Some
               (sprintf
                  "clip_polygon nv_in=%d codes_or=%d c_nv=%d o_nv=%d c_cor=%d o_cor=%d \
                   c_cand=%d o_cand=%d"
                  nv
                  codes_or
                  c_nv
...
let%expect_test "randomized g3_check_normal_facing parity C vs Ox" =
  ...
  |> Sequence.iter ~f:(fun ((vpx, vpy, vpz), (vx, vy, vz), (nx, ny, nz)) ->
    ...
    let ox_res =
      if Ox_3d.g3_check_normal_facing ~view_pos:(vpx, vpy, vpz) (vx, vy, vz) (nx, ny, nz)
      then 1
      else 0
    in
    if c_res <> ox_res
    then (
      ...
      then
        ...
        := Some
             (sprintf
                "g3_check_normal_facing vp=(%d,%d,%d) c=%d ox=%d"
                vpx
                vpy
                vpz
                c_res
...
let%expect_test "randomized calc_rod_corners parity C vs Ox" =
  let rod_fix_gen =
    Quickcheck.Generator.map Int.quickcheck_generator ~f:(fun v ->
      Int.max (-500_000) (Int.min 500_000 v))
  in
  let rod_vec3_gen =
    Quickcheck.Generator.map
      (Quickcheck.Generator.both
         rod_fix_gen
         (Quickcheck.Generator.both rod_fix_gen rod_fix_gen))
      ~f:(fun (x, (y, z)) -> x, y, z)
  in
  let nonzero_fix_gen =
    Quickcheck.Generator.map rod_fix_gen ~f:(fun v -> if v = 0 then 0x10000 else v)
  in
  let width_gen =
    Quickcheck.Generator.map rod_fix_gen ~f:(fun v -> Int.max 1 (Int.abs v))
  in
  let gen =
    Quickcheck.Generator.map
      (Quickcheck.Generator.both
  ...
  |> Sequence.iter ~f:(fun ((bx, by, bz), bw, (tx, ty, tz), tw, msx, msy, msz) ->
    ...
    then (
      incr total;
      let cc0x, cc0y, cc0z, cc1x, cc1y, cc1z, cc2x, cc2y, cc2z, cc3x, cc3y, cc3z, cca =
        c_calc_rod_corners bx by bz bw tx ty tz tw msx msy msz
      in
      let (o0x, o0y, o0z), (o1x, o1y, o1z), (o2x, o2y, o2z), (o3x, o3y, o3z), oca =
      ...
      then (
        ...
        then
          ...
          := Some
               (sprintf
                  "calc_rod_corners c0=(%d,%d,%d)/(%d,%d,%d) o0=(%d,%d,%d)/(%d,%d,%d) \
                   cca=%d oca=%d"
                  cc0x
                  cc0y
                  cc0z
...
let%expect_test "randomized do_facing_check_computed parity C vs Ox" =
  ...
  |> Sequence.iter ~f:(fun ((p0x, p0y, p0z), (p1x, p1y, p1z), (p2x, p2y, p2z)) ->
    ...
    then (
      ...
      then
        ...
        := Some
             (sprintf
                "do_facing_check_computed p0=(%d,%d,%d) p1=(%d,%d,%d) p2=(%d,%d,%d) c=%d \
                 ox=%d"
                p0x
                p0y
                p0z
  ...
  Option.iter !first_mismatch ~f:(fun s -> printf "first_mismatch %s\n" s);
  if !mismatches <> 0
  then failwithf "do_facing_check_computed randomized parity failed" ();
...
let c_compute_center_point_on_side
      (v0x, v0y, v0z)
      (v1x, v1y, v1z)
      (v2x, v2y, v2z)
      (v3x, v3y, v3z)
...
external c_compute_segment_center
  :  int array
  -> int * int * int
...
external c_get_verts_for_normal
  :  int
  -> int
  -> int
  -> int
...
external c_create_abs_vertex_lists
  :  int
  -> int array
  -> int
...
let%expect_test "compute_segment_center parity C vs Ox" =
  ...
  printf
    ...
    (cx = ox && cy = oy && cz = oz);
  [%expect
    {| compute_segment_center c=(32768,32768,32768) ox=(32768,32768,32768) eq=true |}]
;;
...
let%expect_test "create_abs_vertex_lists parity C vs Ox" =
  ...
  List.iter cases ~f:(fun (side_type, sidenum) ->
    ...
    in
    printf
      "create_abs_vertex_lists type=%d side=%d c=(%d,[%d,%d,%d,%d,%d,%d]) \
       ox=(%d,[%d,%d,%d,%d,%d,%d]) eq=%b\n"
      side_type
      sidenum
      cnf
      cv0
...
let%expect_test "extract_orient_from_segment parity C vs Ox" =
  ...
  let (or1, or2, or3), (ou1, ou2, ou3), (of1, of2, of3) =
    Ox_gameseg.extract_orient_from_segment verts_arr
  in
  let eq =
    cr1 = or1
    && cr2 = or2
  ...
  in
  printf
    "extract_orient_from_segment c=[%d,%d,%d;%d,%d,%d;%d,%d,%d] \
     ox=[%d,%d,%d;%d,%d,%d;%d,%d,%d] eq=%b\n"
    cr1
    cr2
    cr3
    cu1
    ...
    eq;
  [%expect
    {| extract_orient_from_segment c=[0,65536,0;-65536,0,0;0,0,65536] ox=[0,65536,0;-65536,0,0;0,0,65536] eq=true |}]
;;

(* ---- FVI parity tests ---- *)

external c_check_point_to_face : int array -> int = "caml_c_check_point_to_face"

external c_find_plane_line_intersection
  :  int array
  -> int * int * int * int
  = "caml_c_find_plane_line_intersection"

external c_check_sphere_to_face : int array -> int = "caml_c_check_sphere_to_face"
...
external c_check_line_to_line : int array -> int * int * int = "caml_c_check_line_to_line"

external c_check_line_to_face
  :  int array
  -> int * int * int * int
  = "caml_c_check_line_to_face"

external c_check_vector_to_sphere_1
  :  int array
  -> int * int * int * int
...
let%expect_test "calc_det_value parity C vs Ox" =
  ...
  List.iter cases ~f:(fun ((rx, ry, rz), (ux, uy, uz), (fx, fy, fz)) ->
    ...
    let ox_res =
      Ox_fvi.calc_det_value ~rvec:(rx, ry, rz) ~uvec:(ux, uy, uz) ~fvec:(fx, fy, fz)
    in
...
let%expect_test "randomized calc_det_value parity C vs Ox" =
  ...
  |> Sequence.iter ~f:(fun ((rx, ry, rz), (ux, uy, uz), (fx, fy, fz)) ->
    ...
    then (
      ...
      then
        first_mismatch
        := Some (sprintf "calc_det_value r=(%d,%d,%d) c=%d ox=%d" rx ry rz c_res ox_res)));
       ...
       ~f:(fun ((p1x, p1y, p1z), (v1x, v1y, v1z), (p2x, p2y, p2z), (v2x, v2y, v2z)) ->
         ...
         then (
           ...
           then
             ...
             := Some
                  (sprintf
                     "check_line_to_line c_ok=%d ox_ok=%d c_t1=%d ox_t1=%d"
                     c_ok
                     ox_ok
                     c_t1
                     ox_t1)));
...
let%expect_test "randomized check_point_to_face parity C vs Ox" =
  ...
  |> Sequence.iter
       ~f:(fun ((cpx, cpy, cpz), (nx, ny, nz), (v0x, v0y, v0z), (v1x, v1y, v1z)) ->
         ...
         then (
           ...
           then
             ...
             := Some
                  (sprintf
                     "check_point_to_face c=%d ox=%d cp=(%d,%d,%d)"
                     c_res
                     ox_res
                     cpx
                     cpy
...
let%expect_test "randomized find_plane_line_intersection parity C vs Ox" =
  ...
  |> Sequence.iter
       ~f:
         (fun
           ((ppx, ppy, ppz), (pnx, pny, pnz), (p0x, p0y, p0z), (p1x, p1y, p1z), rad) ->
         incr total;
         let arr =
           [| ppx; ppy; ppz; pnx; pny; pnz; p0x; p0y; p0z; p1x; p1y; p1z; rad |]
         in
         ...
         let ox_result =
           Ox_fvi.find_plane_line_intersection
             ~plane_pnt:(ppx, ppy, ppz)
             ~plane_norm:(pnx, pny, pnz)
             ~p0:(p0x, p0y, p0z)
             ~p1:(p1x, p1y, p1z)
             ~rad
         in
         let ox_found, ox_nx, ox_ny, ox_nz =
           match ox_result with
           | None -> 0, 0, 0, 0
         ...
         let eq =
           c_found = ox_found
           && (c_found = 0 || (c_nx = ox_nx && c_ny = ox_ny && c_nz = ox_nz))
         in
         if not eq
         then (
           incr mismatches;
           ...
           then
             ...
             := Some
                  (sprintf
                     "find_plane_line_intersection c_found=%d ox_found=%d c=(%d,%d,%d) \
                      ox=(%d,%d,%d)"
                     c_found
                     ox_found
                     c_nx
  ...
  Option.iter !first_mismatch ~f:(fun s -> printf "first_mismatch %s\n" s);
  if !mismatches <> 0
  then failwithf "find_plane_line_intersection randomized parity failed" ();
...
let%expect_test "randomized check_sphere_to_face parity C vs Ox" =
  let rad_gen =
    Quickcheck.Generator.map Int.quickcheck_generator ~f:(fun v ->
      Int.max 1 (Int.abs v land 0xFFFF))
  in
  let gen =
    Quickcheck.Generator.map
      (Quickcheck.Generator.both
       ...
       ~f:
         ...
         then (
           ...
           then
             ...
             := Some
                  (sprintf
                     "check_sphere_to_face c=%d ox=%d pnt=(%d,%d,%d)"
                     c_res
                     ox_res
                     px
                     py
...
let%expect_test "check_line_to_face parity C vs Ox" =
  ...
  let p0 = 0x8000, 0x8000, -0x10000 in
  (* behind the front face *)
  let p1 = 0x8000, 0x8000, 0x10000 in
  (* through the front face *)
  let norm = 0, 0, -0x10000 in
  ...
  let ox_hit, (ox_nx, ox_ny, ox_nz) =
    Ox_fvi.check_line_to_face
      ...
      ~num_faces
      ~vertex_list
      ~seg_verts:test_seg_verts
      ~seg_vert_positions:test_seg_vert_positions
  in
  let eq =
    c_hit = ox_hit && (c_hit = 0 || (c_nx = ox_nx && c_ny = ox_ny && c_nz = ox_nz))
  in
  printf
    "check_line_to_face hit c=%d ox=%d newp c=(%d,%d,%d) ox=(%d,%d,%d) eq=%b\n"
    c_hit
    ox_hit
    c_nx
    c_ny
    ...
    eq;
  [%expect
    {| check_line_to_face hit c=1 ox=1 newp c=(32768,32768,0) ox=(32768,32768,0) eq=true |}]
;;

let%expect_test "randomized check_vector_to_sphere_1 parity C vs Ox" =
  let sphere_rad_gen =
    Quickcheck.Generator.map Int.quickcheck_generator ~f:(fun v ->
      Int.max 0x100 (Int.abs v land 0x7FFFF))
  in
  let gen =
    Quickcheck.Generator.map
      (Quickcheck.Generator.both
  ...
  |> Sequence.iter ~f:(fun ((p0x, p0y, p0z), (p1x, p1y, p1z), (spx, spy, spz), srad) ->
    ...
    in
    let eq =
      c_dist = ox_dist && (c_dist = 0 || (c_ix = ox_ix && c_iy = ox_iy && c_iz = ox_iz))
    in
    if not eq
    then (
      incr mismatches;
      ...
      then
        ...
        := Some
             (sprintf
                "check_vector_to_sphere_1 c_dist=%d ox_dist=%d c=(%d,%d,%d) ox=(%d,%d,%d)"
                c_dist
                ox_dist
                c_ix
                c_iy
  ...
  Option.iter !first_mismatch ~f:(fun s -> printf "first_mismatch %s\n" s);
  if !mismatches <> 0
  then failwithf "check_vector_to_sphere_1 randomized parity failed" ();
...
let%expect_test "evaluate_curve randomized parity C vs Ox" =
  let eq =
    Ox_curves.create_curve
      ~p1:(0x10000, 0x20000, 0x30000)
      ~p4:(0x40000, 0x50000, 0x60000)
      ~r1:(0x10000, 0x10000, 0x10000)
      ~r4:(-0x10000, -0x10000, -0x10000)
  in
  let t_gen =
    Quickcheck.Generator.map Int.quickcheck_generator ~f:(fun v -> Int.abs v land 0xFFFF)
  in
  let total = ref 0 in
  let mismatches = ref 0 in
  ...
  |> Sequence.iter ~f:(fun t ->
    ...
    let c = c_evaluate_curve arr in
    let ox = Ox_curves.evaluate_curve eq ~t in
    if not ([%compare.equal: int * int * int] c ox) then incr mismatches);
...
let%expect_test "physics_turn_towards_vector zero goal returns unchanged rotvel" =
  ...
  let ox =
    Ox_physics.physics_turn_towards_vector
      ...
      ~rate:0x10000
      ~is_morph:false
      ~cur_rotvel:(500, 600, 700)
  in
  printf
    "c=(%d,%d,%d) ox=(%d,%d,%d) eq=%b\n"
    (let a, _, _ = c in
     a)
    (let _, a, _ = c in
     a)
...
let%expect_test "randomized physics_turn_towards_vector parity C vs Ox" =
  let rate_gen =
    Quickcheck.Generator.map Int.quickcheck_generator ~f:(fun v ->
      ...
      if v = 0 then 0x10000 else v)
  in
  let bool_gen =
    Quickcheck.Generator.map Int.quickcheck_generator ~f:(fun v -> v land 1)
  in
  let gen =
    Quickcheck.Generator.map
      (Quickcheck.Generator.both
  ...
  |> Sequence.iter
       ~f:(fun ((gx, gy, gz), (fx, fy, fz), rate, is_morph, (crx, cry, crz)) ->
         incr total;
         let c =
           c_physics_turn_towards_vector gx gy gz fx fy fz rate is_morph crx cry crz
         in
         let ox =
           Ox_physics.physics_turn_towards_vector
             ~goal:(gx, gy, gz)
         ...
         in
         if not ([%compare.equal: int * int * int] c ox)
         then (
           incr mismatches;
           if Option.is_none !first_mismatch
           then
             ...
             := Some
                  (sprintf
                     "goal=(%d,%d,%d) fvec=(%d,%d,%d) rate=%d morph=%d rotvel=(%d,%d,%d) \
                      c=(%d,%d,%d) ox=(%d,%d,%d)"
                     gx
                     gy
                     gz
...
let ox_do_physics_sim_rot
  ...
  | Some (orient, rotvel, turnroll) -> 1, orient, rotvel, turnroll
;;

let print_sim_rot_result
      label
      (tag, ((r1, r2, r3), (u1, u2, u3), (f1, f2, f3)), (rvx, rvy, rvz), turnroll)
  =
  if tag = 0
  then printf "%s=no_change " label
  else
    printf
      "%s=(orient=[%d,%d,%d;%d,%d,%d;%d,%d,%d] rv=(%d,%d,%d) tr=%d) "
      label
      r1
      r2
      r3
...
let%expect_test "do_physics_sim_rot simple rotation no drag" =
  ...
  printf "eq=%b\n" eq;
  [%expect
    {| c=(orient=[0,0,-65536;0,65536,0;65536,0,0] rv=(0,16384,0) tr=0) ox=(orient=[0,0,-65536;0,65536,0;65536,0,0] rv=(0,16384,0) tr=0) eq=true |}]
;;
...
let%expect_test "do_physics_sim_rot with drag no thrust" =
  ...
  printf "eq=%b\n" eq;
  [%expect
    {| c=(orient=[65536,0,0;0,65497,2208;0,-2208,65497] rv=(22528,0,0) tr=0) ox=(orient=[65536,0,0;0,65497,2208;0,-2208,65497] rv=(22528,0,0) tr=0) eq=true |}]
;;
...
let%expect_test "randomized do_physics_sim_rot parity C vs Ox" =
  ...
  for _ = 1 to 5000 do
    ...
    then (
      ...
      if Option.is_none !first_mismatch
      then (
        let ct, ((cr1, cr2, cr3), (cu1, cu2, cu3), (cf1, cf2, cf3)), (crx, cry, crz), ctr =
          c
        in
        let ot, ((or1, or2, or3), (ou1, ou2, ou3), (of1, of2, of3)), (orx, ory, orz), otr =
          ox
        in
        first_mismatch
        := Some
             (sprintf
                "rv=(%d,%d,%d) rt=(%d,%d,%d) drag=%d mass=%d flags=%d tr=%d ft=%d \
                 c=(tag=%d orient=[%d,%d,%d;%d,%d,%d;%d,%d,%d] rv=(%d,%d,%d) tr=%d) \
                 ox=(tag=%d orient=[%d,%d,%d;%d,%d,%d;%d,%d,%d] rv=(%d,%d,%d) tr=%d)"
                rvx
                rvy
                rvz
                rtx
...
external c_calc_gun_point : int array -> int * int * int = "caml_c_calc_gun_point"

let pack_gun_point_args
...
let%expect_test "calc_gun_point with one hierarchy level" =
  ...
  let n = 10 in
  let anim_angles = Array.init n ~f:(fun _ -> 0, 0, 0) in
  let offsets = Array.init n ~f:(fun i -> if i = 1 then 0x10000, 0, 0 else 0, 0, 0) in
  let parents = Array.init n ~f:(fun _ -> 0) in
  let orient = (0x10000, 0, 0), (0, 0x10000, 0), (0, 0, 0x10000) in
...
let%expect_test "calc_gun_point with two hierarchy levels" =
  ...
  let offsets =
    Array.init n ~f:(fun i ->
      if i = 2 then 0, 0x4000, 0 else if i = 1 then 0x10000, 0, 0 else 0, 0, 0)
  in
  let parents = Array.init n ~f:(fun i -> if i = 2 then 1 else 0) in
  let orient = (0x10000, 0, 0), (0, 0x10000, 0), (0, 0, 0x10000) in
  let pos = 0, 0, 0 in
  let packed =
    pack_gun_point_args
      ~gun_point:(0x8000, 0, 0)
      ~start_mn:2
      ~anim_angles
      ~offsets
      ~parents
      ...
      then (
        ...
        := Some
             (sprintf
                ...
                (let x, _, _ = gun_point in
                 x)
                (let _, y, _ = gun_point in
                 y)
                (let _, _, z = gun_point in
                 z)
                start_mn
                cx
                cy
  ...
  let ox =
    let (rx, ry, rz), skip_ai =
      Ox_physics.phys_apply_rot
        ...
        ~fvec:(0x10000, 0, 0)
        ~is_morph:false
        ~cur_rotvel:(100, 200, 0)
    in
    rx, ry, rz, if skip_ai then 1 else 0
  in
  let cx, cy, cz, cs = c in
  let oxx, oxy, oxz, oxs = ox in
  printf
    "c=(%d,%d,%d,%d) ox=(%d,%d,%d,%d) eq=%b\n"
    cx
    cy
    cz
    cs
  ...
  for _ = 1 to 5000 do
    ...
    then (
      ...
      then (
        ...
        := Some
             (sprintf
                "f=(%d,%d,%d) mass=%d robot=%d fvec=(%d,%d,%d) morph=%d rv=(%d,%d,%d) \
                 c=(%d,%d,%d,%d) ox=(%d,%d,%d,%d)"
                fx
                fy
                fz
                mass
...
let%expect_test "randomized ai_turn_towards_vector parity C vs Ox (D1, no seismic)" =
  ...
  for _ = 1 to 5000 do
    ...
    incr total;
    let c =
      c_ai_turn_towards_vector gx gy gz fx fy fz rx ry rz rate frame_time 0 0 0 0 0
    in
    let ox =
    ...
    then (
      ...
      then (
        ...
        := Some
             (sprintf
                "g=(%d,%d,%d) f=(%d,%d,%d) r=(%d,%d,%d) rate=%d ft=%d \
                 c=(%d,%d,%d,%d,%d,%d,%d,%d,%d) ox=(%d,%d,%d,%d,%d,%d,%d,%d,%d)"
                gx
                gy
                gz
                fx
...
let%expect_test "randomized ai_turn_towards_vector parity C vs Ox (D2, with seismic)" =
  ...
  for _ = 1 to 5000 do
    ...
    then (
      ...
      then (
        ...
        := Some
             (sprintf
                "g=(%d,%d,%d) f=(%d,%d,%d) r=(%d,%d,%d) rate=%d ft=%d sm=%d rm=%d \
                 rv=(%d,%d,%d) c=(%d,%d,%d,%d,%d,%d,%d,%d,%d) \
  ...
  printf
    "ai_turn_towards_vector seismic random total=%d mismatches=%d\n"
    !total
    !mismatches;
  Option.iter !first_mismatch ~f:(fun s -> printf "first_mismatch %s\n" s);
...
let%expect_test "set_thrust_from_velocity zero velocity" =
  ...
  let ox =
    Ox_physics.set_thrust_from_velocity ~mass:0x10000 ~drag:0x8000 ~velocity:(0, 0, 0)
  in
  printf
    "c=(%d,%d,%d) ox=(%d,%d,%d) eq=%b\n"
    (let x, _, _ = c in
...
let%expect_test "randomized set_thrust_from_velocity parity C vs Ox" =
  ...
  for _ = 1 to 5000 do
    ...
    let ox = Ox_physics.set_thrust_from_velocity ~mass ~drag ~velocity:(vx, vy, vz) in
    if Poly.(c <> ox)
    then (
      incr mismatches;
      if Option.is_none !first_mismatch
...
let%expect_test "move_towards_vector zero goal" =
  let c = c_move_towards_vector 0x10000 0 0 0 0 0 0x10000 0 0 0x1000 0 0x20000 0 1 0 0 in
  let ox =
  ...
  for _ = 1 to 5000 do
    ...
    then (
      ...
      then (
        ...
        := Some
             (sprintf
                "v=(%d,%d,%d) g=(%d,%d,%d) f=(%d,%d,%d) ft=%d diff=%d ms=%d at=%d db=%d \
                 th=%d kam=%d c=(%d,%d,%d) ox=(%d,%d,%d)"
                vx
                vy
                vz
                gx
...
external c_move_around_player : int array -> int * int * int = "caml_c_move_around_player"

let%expect_test "move_around_player fast_flag=0 returns unchanged" =
  let packed =
    [| 100; 200; 300; 0; 0; 0; 0; 0; 0x10000; 0x1000; 0; 5; 0; 0; 0; 0; 0x20000; 0; 0 |]
  in
...
let%expect_test "randomized move_around_player parity C vs Ox" =
  ...
  for _ = 1 to 5000 do
    ...
    let fast_flag = Random.State.int state 5 - 1 in
    (* -1 to 3 *)
...
let%expect_test "move_away_from_player no attack" =
  let c =
    c_move_away_from_player 0 0 0 0x10000 0 0 0 0x10000 0 0x10000 0 0 0x1000 0 5 0 0x20000
  in
  let ox =
    Ox_physics.move_away_from_player
      ~velocity:(0, 0, 0)
      ~vec_to_player:(0x10000, 0, 0)
      ~uvec:(0, 0x10000, 0)
      ~rvec:(0x10000, 0, 0)
      ~frame_time:0x1000
      ~frame_count:0
...
external c_set_object_turnroll : int -> int -> int -> int = "caml_c_set_object_turnroll"

let%expect_test "set_object_turnroll zero rotvel" =
  let c = c_set_object_turnroll 0 0 0x1000 in
  let ox = Ox_physics.set_object_turnroll ~rotvel_y:0 ~turnroll:0 ~frame_time:0x1000 in
...
let%expect_test "lead_player cloaked returns failure" =
  let c = c_lead_player 0 0 0 0x100000 0 0 0x40000 0 0 0x10000 0 0 1 0x80000 0 2 in
  let ox =
    match
      Ox_physics.lead_player
        ~fire_point:(0, 0, 0)
        ~believed_player_pos:(0x100000, 0, 0)
        ~player_velocity:(0x40000, 0, 0)
        ~fvec:(0x10000, 0, 0)
        ~player_cloaked:true
        ~max_weapon_speed:0x80000
        ~is_matter:false
        ~difficulty_level:2
    with
    | None -> 0, 0, 0, 0
...
let%expect_test "randomized lead_player parity C vs Ox" =
  ...
  for _ = 1 to 5000 do
    ...
    let fpy = gen_fix () in
    let fpz = gen_fix () in
    let bpx = fpx + gen_fix () in
    let bpy = fpy + gen_fix () in
    let bpz = fpz + gen_fix () in
    let pvx = gen_vel () in
    let pvy = gen_vel () in
    let pvz = gen_vel () in
    let fvx = gen_fix () in
    ...
    then (
      ...
      then (
        ...
        := Some
             (sprintf
                "fp=(%d,%d,%d) bp=(%d,%d,%d) pv=(%d,%d,%d) cloak=%d ws=%d mat=%d diff=%d \
                 c=(%d,%d,%d,%d) ox=(%d,%d,%d,%d)"
                fpx
                fpy
                fpz
...
external c_homing_missile_turn_towards_velocity
  ...
  -> int
  -> int * int * int * int * int * int * int * int * int
  = "caml_c_homing_missile_turn_towards_velocity_bc"
    "caml_c_homing_missile_turn_towards_velocity"

let%expect_test "homing_missile_turn_towards_velocity zero velocity" =
  let c = c_homing_missile_turn_towards_velocity 0 0 0 0 0 0x10000 0x1000 in
  ...
  printf
    "c=(%d,%d,%d,%d,%d,%d,%d,%d,%d) ox=(%d,%d,%d,%d,%d,%d,%d,%d,%d) eq=%b\n"
    (let a, _, _, _, _, _, _, _, _ = c in
     a)
    (let _, b, _, _, _, _, _, _, _ = c in
     b)
    (let _, _, c_, _, _, _, _, _, _ = c in
     c_)
    (let _, _, _, d, _, _, _, _, _ = c in
     d)
    (let _, _, _, _, e, _, _, _, _ = c in
     e)
    (let _, _, _, _, _, f, _, _, _ = c in
     f)
    (let _, _, _, _, _, _, g, _, _ = c in
     g)
    (let _, _, _, _, _, _, _, h, _ = c in
     h)
    (let _, _, _, _, _, _, _, _, i = c in
     i)
    (let a, _, _, _, _, _, _, _, _ = ox in
     a)
    (let _, b, _, _, _, _, _, _, _ = ox in
     b)
    (let _, _, c_, _, _, _, _, _, _ = ox in
     c_)
    (let _, _, _, d, _, _, _, _, _ = ox in
     d)
    (let _, _, _, _, e, _, _, _, _ = ox in
     e)
    (let _, _, _, _, _, f, _, _, _ = ox in
     f)
    (let _, _, _, _, _, _, g, _, _ = ox in
     g)
    (let _, _, _, _, _, _, _, h, _ = ox in
     h)
    (let _, _, _, _, _, _, _, _, i = ox in
     i)
    Poly.(c = ox);
  [%expect
    {| c=(65536,0,0,0,65536,0,0,0,65536) ox=(65536,0,0,0,65536,0,0,0,65536) eq=true |}]
;;
...
let%expect_test "randomized homing_missile_turn_towards_velocity parity C vs Ox" =
  ...
  printf
    "homing_missile_turn_towards_velocity random total=%d mismatches=%d\n"
    !total
    !mismatches;
  Option.iter !first_mismatch ~f:(fun s -> printf "first_mismatch %s\n" s);
  if !mismatches <> 0
  then failwithf "homing_missile_turn_towards_velocity parity failed" ();
...
let%expect_test "do_physics_align_object: identity orient, up normal on side 3" =
  ...
  printf
    ...
    c_result.(10);
  printf
    "Ox: tag=%d orient=(%d,%d,%d,%d,%d,%d,%d,%d,%d) fl=%d\n"
    oc_tag
    orx
    ory
    orz
...
let%expect_test "randomized do_physics_align_object parity C vs Ox" =
  ...
  for _ = 1 to 5000 do
    ...
    let num_faces =
      Array.init 6 ~f:(fun _ -> if Random.State.int state 32768 > 16384 then 2 else 1)
    in
    ...
    then (
      ...
      then
        ...
        := Some
             (sprintf
                "tag C=%d Ox=%d fl C=%d Ox=%d orient C=(%d,%d,%d,%d,%d,%d,%d,%d,%d) \
                 Ox=(%d,%d,%d,%d,%d,%d,%d,%d,%d)"
                c_result.(0)
                oc_tag
                c_result.(10)
...
let%expect_test "check_vector_to_object: robot with attack_type gets 3/4 size" =
  ...
  let c =
    c_check_vector_to_object
      0
      0
      (-f1_0 * 2)
      0
      0
      0 (* p0 behind, p1 at origin *)
      0 (* rad=0 *)
      0
      0
      0 (* obj_pos = origin *)
      f1_0
      2
      1 (* size, OBJ_ROBOT, attack_type=1 *)
      0
      0
      0 (* otherobj_type, game_mode_coop=false, parent_type *)
  in
  let ox =
    Ox_fvi.check_vector_to_object
      ...
      ~attack_type:1
      ~otherobj_type:0
      ~game_mode_coop:false
      ~otherobj_parent_type:0
  in
...
let%expect_test "check_vector_to_object: player vs player gets half size" =
  ...
  let c =
    c_check_vector_to_object
      ...
      f1_0
      4
      0 (* size, OBJ_PLAYER, attack_type=0 *)
      4
      0
      0 (* otherobj_type=OBJ_PLAYER, game_mode_coop=false, parent *)
  in
  let ox =
    Ox_fvi.check_vector_to_object
      ~p0:(0, 0, -f1_0 * 2)
...
let%expect_test "set_next_fire_time_d1: random parity" =
  ...
  for _ = 1 to 5000 do
    ...
    let orfc, onf =
      Ox_ai.set_next_fire_time_d1
        ~rapidfire_count:rfc
        ~rapidfire_count_limit:rfc_limit
        ~firing_wait:fw
    in
    incr total;
    if crfc <> orfc || cnf <> onf
    then (
      incr mismatches;
      if Option.is_none !first_mismatch
      then
        first_mismatch := Some (sprintf "rfc C=%d Ox=%d nf C=%d Ox=%d" crfc orfc cnf onf))
  done;
...
let%expect_test "set_next_fire_time_d2: secondary weapon fire" =
  ...
  printf
    "C: rfc=%d nf=%d nf2v=%d nf2=%d  Ox: rfc=%d nf=%d nf2v=%d nf2=%d\n"
    crfc
    cnf
    cnf2v
    cnf2
...
let%expect_test "set_next_fire_time_d2: random parity" =
  ...
  for _ = 1 to 5000 do
    ...
    let wt2 = Random.State.int state 20 - 1 in
    (* -1 to 18 *)
    let beh = Random.State.int state 8 in
    let prand = Random.State.int state 32768 in
    let crfc, cnf, cnf2v, cnf2 =
      c_set_next_fire_time_d2 rfc rfc_limit fw fw2 gun_num wt2 beh prand
    in
    ...
    in
    incr total;
    if crfc <> orfc || cnf <> onf || cnf2v <> onf2v || cnf2 <> onf2
    then (
      incr mismatches;
      if Option.is_none !first_mismatch
      then
...
let%expect_test "compute_headlight_light_d2: headlight on" =
  ...
  let c =
    c_compute_headlight_light_d2 0 0 (f1_0 * 10) f1_0 (f1_0 / 2) pflags (f1_0 * 50) 1
  in
  let o =
...
let%expect_test "compute_headlight_light_d2: random parity" =
  ...
  for _ = 1 to 5000 do
    ...
    let pflags =
      ...
      lor if Random.State.int state 2 = 1 then pf_headlight_on else 0
    in
...
let%expect_test "ai_behavior_to_mode_d1: all behaviors" =
  ...
  List.iter cases ~f:(fun b ->
    ...
    printf "beh=0x%02x C=%d Ox=%d %s\n" b c o (if c = o then "OK" else "MISMATCH"));
  [%expect
    {|
...
let%expect_test "ai_turn_randomly: random parity" =
  ...
  for _ = 1 to 5000 do
    ...
    then (
      incr mismatches;
      if Option.is_none !first_mismatch
      then first_mismatch := Some (sprintf "C=(%d,%d,%d) Ox=(%d,%d,%d)" cx cy cz ox oy oz))
  done;
...
let%expect_test "get_explosion_vclip: all cases" =
  ...
  List.iter cases ~f:(fun (ot, st, e1, e2, ep, label) ->
    ...
    let o =
      Ox_collide.get_explosion_vclip
        ~obj_type:ot
        ~stage:st
        ~exp1_vclip_num:e1
        ~exp2_vclip_num:e2
        ~expl_vclip_num:ep
    in
    printf "%s: C=%d Ox=%d %s\n" label c o (if c = o then "OK" else "MISMATCH"));
  [%expect
    {|
