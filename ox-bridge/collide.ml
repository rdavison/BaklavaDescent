    ...
    Effect.Deep.match_with
      (fun () ->
         Ox_collide.apply_damage_to_robot_d1
           ...
           ~damage
           ~is_boss:(is_boss <> 0)
           ~is_multiplayer:(is_multiplayer <> 0)
           ~obj_id
           ~killer_objnum)
      ()
      { retc = (fun x -> x)
...
let cd_get_explosion_vclip obj_type stage exp1_vclip_num exp2_vclip_num expl_vclip_num =
...
external effect_query_player_dead_or_no_shields
  :  unit
  -> bool
  = "cd_ox_effect_query_player_dead_or_no_shields"

external effect_query_multi_all_players_alive
  :  unit
  -> bool
  = "cd_ox_effect_query_multi_all_players_alive"

external effect_do_final_boss_hacks : unit -> unit = "cd_ox_effect_do_final_boss_hacks"

external effect_multi_send_finish_game
  :  unit
  -> unit
  = "cd_ox_effect_multi_send_finish_game"
...
external effect_multi_explode_d2
  :  int
  -> int
  -> bool
  -> bool
  = "cd_ox_effect_multi_explode_d2"
...
external effect_multi_send_robot_explode_d2
  ...
  -> bool
  -> unit
  = "cd_ox_effect_multi_send_robot_explode_d2"

external effect_start_robot_death_sequence
  :  int
  -> unit
  = "cd_ox_effect_start_robot_death_sequence"

external effect_special_reactor_stuff
  :  unit
  -> unit
  = "cd_ox_effect_special_reactor_stuff"
    ...
    Effect.Deep.match_with
      (fun () ->
         Ox_collide.apply_damage_to_robot_d2
           ...
           ~is_thief:(is_thief <> 0)
           ~is_death_roll:(is_death_roll <> 0)
           ~is_kamikaze:(is_kamikaze <> 0)
           ~robot_id
           ~is_multiplayer:(is_multiplayer <> 0)
           ~is_final_level:(is_final_level <> 0)
           ~obj_id
           ~killer_objnum)
      ()
      { retc = (fun x -> x)
...
external effect_explode_object_delay_clutter
  :  int
  -> int
  -> unit
...
external effect_show_hud_invul_message
  :  unit
  -> unit
  = "cd_ox_effect_show_hud_invul_message"

external effect_controlcen_been_hit : unit -> unit = "cd_ox_effect_controlcen_been_hit"

external effect_do_controlcen_destroyed
  :  int
  -> unit
  = "cd_ox_effect_do_controlcen_destroyed"

external effect_add_controlcen_score : unit -> unit = "cd_ox_effect_add_controlcen_score"

external effect_multi_send_destroy_controlcen
  :  int
  -> int
  -> unit
  = "cd_ox_effect_multi_send_destroy_controlcen"

external effect_sound_controlcen_destroyed
  :  int
  -> unit
  = "cd_ox_effect_sound_controlcen_destroyed"

external effect_explode_object_delay_controlcen
  :  int
  -> int
  -> unit
    ...
    Effect.Deep.match_with
      (fun () ->
         Ox_collide.apply_damage_to_controlcen
           ...
           ~flags
           ~damage
           ~who_is_player:(who_is_player <> 0)
           ~who_is_local_player:(who_is_local_player <> 0)
           ~who_objnum
           ~local_player_objnum
           ~is_multiplayer:(is_multiplayer <> 0)
           ~is_coop:(is_coop <> 0)
...
external effect_palette_flash_d1
  :  int
  -> int
  -> int
  -> unit
    ...
    Effect.Deep.match_with
      (fun () ->
         Ox_collide.apply_damage_to_player_d1
           ~is_dead:(is_dead <> 0)
           ~is_invulnerable:(is_invulnerable <> 0)
           ~is_endlevel:(is_endlevel <> 0)
...
external effect_palette_flash_d2
  :  int
  -> int
  -> int
  -> unit
...
let cd_calc_best_gun packed = Ox_collide.calc_best_gun packed

(* -- maybe_kill_weapon (pure, no effects) ------------------------------ *)

let cd_maybe_kill_weapon_d1 weapon_id phys_flags weapon_shields other_type other_shields =
  let new_shields, dead =
