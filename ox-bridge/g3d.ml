...
let cd_scale_matrix r1 r2 r3 u1 u2 u3 f1 f2 f3 wsx wsy wsz zoom =
  let ((or1, or2, or3), (ou1, ou2, ou3), (of1, of2, of3)), (msx, msy, msz), _unscaled =
...
let cd_g3_start_instance_matrix
  ...
  in
  let (nvpx, nvpy, nvpz), ((nr1, nr2, nr3), (nu1, nu2, nu3), (nf1, nf2, nf3)) =
...
let cd_g3_check_normal_facing vpx vpy vpz vx vy vz nx ny nz =
  let facing =
    Ox_3d.g3_check_normal_facing ~view_pos:(vpx, vpy, vpz) (vx, vy, vz) (nx, ny, nz)
  in
...
let cd_clip_polygon codes_or codes_and (arr : int array) =
  ...
  let points =
    List.init nv ~f:(fun i ->
      ...
      , arr.(b + 7) ))
  in
  let clipped, final_or, final_and = Ox_3d.clip_polygon ~codes_or ~codes_and points in
