...
uint32_t check_point_to_face(vms_vector * checkp, segment * sp, side * s, int facenum, int nv, int* vertex_list)
{
	...
	for (edge = edgemask = 0; edge < nv; edge++) 
	{
		...
		lv = &Vertices[vertex_list[facenum * 3 + edge]];
		v0.xyz[0] = lv->x; v0.xyz[1] = lv->y; v0.xyz[2] = lv->z;
		lv = &Vertices[vertex_list[facenum * 3 + ((edge + 1) % nv)]];
		v1.xyz[0] = lv->x; v1.xyz[1] = lv->y; v1.xyz[2] = lv->z;
...
int fvi_sub(vms_vector* intp, int* ints, vms_vector* p0, int startseg, vms_vector* p1, fix rad, short thisobjnum, int* ignore_obj_list, int flags, int* seglist, int* n_segs, int entry_seg)
{
	...
	startmask = 0; endmask = 0; centermask = 0;
	masks.centermask = 0; masks.facemask = 0; masks.sidemask = 0;
			...
			for (face = 0; face < 2; face++, bit <<= 1) 
			{
				...
				{            
					...
					if (startmask & bit)		//start was also though.  Do extra check
						face_hit_type = special_check_line_to_face(&hit_point, p0, p1, seg, side, face, ((num_faces == 1) ? 4 : 3), rad);
					else
						face_hit_type = check_line_to_face(&hit_point, p0, p1, seg, side, face, ((num_faces == 1) ? 4 : 3), rad);
