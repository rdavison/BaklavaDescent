	...
	for (render_seg = 0; render_seg < N_render_segs; render_seg++)
	{
		...
		while (objnum != -1)
		{
			...
			case OBJ_WEAPON:
				...
				if (obj->id == FLARE_ID)
					obj_intensity = 2 * (std::min(obj_intensity, obj->lifeleft) + ((GameTime ^ Obj_light_xlate[objnum & 0x0f]) & 0x3fff));
				break;
			case OBJ_POWERUP:
...
fix compute_object_light(object* obj, vms_vector* rotated_pnt)
{
	...
	int32_t packed[8];
	packed[0] = Segments[obj->segnum].static_light;
	packed[1] = object_light[objnum];
	packed[2] = object_id[objnum];
	packed[3] = obj->id;
	packed[4] = FrameTime;
	packed[5] = (Viewer != old_viewer) ? 1 : 0;
	packed[6] = compute_headlight_light(rotated_pnt, f1_0);
