...
void ai_turn_towards_vector(vms_vector* goal_vector, object* objp, fix rate)
{
	...
	if (!ox_bridge_logged)
	{
		fprintf(stderr, "[OX] ai_turn_towards_vector using cd_ox_ai_turn_towards_vector.\n");
		ox_bridge_logged = 1;
	}
...
int do_silly_animation(object* objp)
{
...
#ifdef USE_OX_BRIDGE
	{
		...
		if (!ox_logged) { fprintf(stderr, "[OX] do_silly_animation (D1)\n"); ox_logged = 1; }
		...
		int32_t packed[512]; /* generous upper bound */
		int idx = 0;
		packed[idx++] = num_guns;
		...
		/* Pack joint entries grouped by gun */
		for (gun_num = 0; gun_num <= num_guns; gun_num++) {
		...
		/* Pack current anim_angles for all model joints */
		...
		/* Pack current goal_angles for all model joints */
		for (int j = 0; j < model_n_models; j++) {
		...
		/* Pack current delta_angles for all model joints */
		for (int j = 0; j < model_n_models; j++) {
		...
		/* Pack achieved_state and goal_state */
		for (gun_num = 0; gun_num <= num_guns; gun_num++)
			packed[idx++] = Ai_local_info[objnum].achieved_state[gun_num];
		for (gun_num = 0; gun_num <= num_guns; gun_num++)
			packed[idx++] = Ai_local_info[objnum].goal_state[gun_num];

		/* Output: at_goal(1) + goal_angles(model_n_models*3) + delta_angles(model_n_models*3)
		   + achieved_state(n_guns_plus_1) + goal_state(n_guns_plus_1) */
...
void do_firing_stuff(object* obj, int player_visibility, vms_vector* vec_to_player)
{
	...
	{
		static int ox_logged = 0;
		if (!ox_logged) { fprintf(stderr, "[OX] do_firing_stuff (D1)\n"); ox_logged = 1; }
		int objnum = obj - Objects;
		...
		packed[11] = 0; /* D1: no dist_to_last_fired check */
		int32_t out[3];
...
void compute_vis_and_vec(object* objp, vms_vector* pos, ai_local* ailp, vms_vector* vec_to_player, int* player_visibility, robot_info* robptr, int* flag)
{
	...
	if (!*flag) {
		...
		*player_visibility = out[0];
		...
		// Update pos if pviso modified it
...
void do_ai_frame(object* obj)
{
	...
	ai_state[15] = 0;                 // next_fire2 (D2 only)
	...
	rinfo[2]  = -1;                   // weapon_type2 (D2 only)
	rinfo[3]  = robptr->n_guns;
	rinfo[4]  = robptr->boss_flag;
	rinfo[5]  = 0;                    // companion (D2 only)
	rinfo[6]  = 0;                    // thief (D2 only)
	rinfo[7]  = 0;                    // kamikaze (D2 only)
	...
	// OCaml-authoritative: effects fire normally, write back result, skip C path
	{
		static int ox_logged = 0;
		if (!ox_logged) {
			fprintf(stderr, "[OX] do_ai_frame (D1) OCaml-authoritative.\n");
			ox_logged = 1;
		}
...
void process_awareness_events(void)
{
	...
	if (cd_ox_is_ready()) {
		...
		cd_ox_process_awareness_events(
			Num_awareness_events, Highest_segment_index, 0, 0,
...
void set_player_awareness_all(void)
{
	...
	if (cd_ox_is_ready()) {
		...
		for (int i = 0; i < num_objs; i++) {
			...
			obj_sub_flags[i] = 0; /* D1: no SUB_FLAGS */
			out_awareness_times[i] = -1;
		}
