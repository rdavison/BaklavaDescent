...
int find_exit_side(object* obj); //[ISB] ugh
...
int chase_angles(vms_angvec* cur_angles, vms_angvec* desired_angles)
{
	...
	cur_angles->p = (fixang)out_p;
	cur_angles->b = (fixang)out_b;
	cur_angles->h = (fixang)out_h;
	return mask;
#else
...
void do_endlevel_frame()
{
	...
	if (!outside_mine)
	{
		...
		if ((explosion_wait1 -= FrameTime) < 0) 
		{
			...
			explosion_wait1 = 0x2000 + P_Rand() / 4;
	...
	if (Endlevel_sequence >= EL_FLYTHROUGH && Endlevel_sequence < EL_OUTSIDE)
		if ((explosion_wait2 -= FrameTime) < 0)
		{
			...
			vm_vec_copy_scale(&tpnt, &ConsoleObject->orient.rvec, (P_Rand() - PRAND_MAX / 2) * 100);
			vm_vec_scale_add2(&tpnt, &ConsoleObject->orient.uvec, (P_Rand() - PRAND_MAX / 2) * 100);
			vm_vec_add2(&tpnt, &ConsoleObject->pos);

			if (Endlevel_sequence == EL_FLYTHROUGH)
				vm_vec_scale_add2(&tpnt, &ConsoleObject->orient.fvec, P_Rand() * 200);
			else
				vm_vec_scale_add2(&tpnt, &ConsoleObject->orient.fvec, P_Rand() * 60);
...
void do_endlevel_flythrough(int n)
{
	...
	if (flydata->first_time || obj->segnum != old_player_seg) //moved into new seg
	{
		...
		if (seg_time) 
		{
			flydata->angstep.x = std::max<fix>(-MAX_ANGSTEP, std::min<fix>(MAX_ANGSTEP, fixdiv(delta_ang(flydata->angles.p, dest_angles.p), seg_time)));
			flydata->angstep.z = std::max<fix>(-MAX_ANGSTEP, std::min<fix>(MAX_ANGSTEP, fixdiv(delta_ang(flydata->angles.b, dest_angles.b), seg_time)));
			flydata->angstep.y = std::max<fix>(-MAX_ANGSTEP, std::min<fix>(MAX_ANGSTEP, fixdiv(delta_ang(flydata->angles.h, dest_angles.h), seg_time)));
