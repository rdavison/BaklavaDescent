...
int find_point_seg(vms_vector* p, int segnum)
{
	...
	packed[3] = segnum;
	packed[4] = n_segments;
	packed[5] = 0; // D1: Doing_lighting_hack_flag is always 0 at runtime
...
fix find_connected_distance(vms_vector* p0, int seg0, vms_vector* p1, int seg1, int max_depth, int wid_flag)
{
	...
	if (!ox_bridge_logged)
	{
		fprintf(stderr, "[OX] find_connected_distance using cd_ox_find_connected_distance.\n");
		ox_bridge_logged = 1;
	}
	...
	packed[9] = wid_flag;
	packed[10] = n_segs;
	packed[11] = 0; // D1: don't check WID on adjacency shortcut
	// Per-segment data
	for (int s = 0; s < n_segs; s++)
	{
		...
		for (int i = 0; i < 6; i++)
			packed[base + i] = segp->children[i];
		for (int i = 0; i < 6; i++)
			packed[base + 6 + i] = WALL_IS_DOORWAY(segp, i);
		vms_vector center;
...
void validate_removable_wall(segment* sp, int sidenum, int tmap_num)
{
	...
	packed[17] = tmap_num;
	int32_t out_buf[8];
	cd_ox_validate_removable_wall(packed, 18, out_buf);
	sp->sides[sidenum].type = out_buf[0];
	sp->sides[sidenum].normals[0].x = out_buf[1];
	sp->sides[sidenum].normals[0].y = out_buf[2];
	sp->sides[sidenum].normals[0].z = out_buf[3];
