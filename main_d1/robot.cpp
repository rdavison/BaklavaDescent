...
int robot_get_anim_state(jointpos** jp_list_ptr, int robot_type, int gun_num, int state)
{
...
#ifdef USE_OX_BRIDGE
	{
		...
		/* Pack anim_states flat: [g][state] -> (n_joints, offset) */
		int rj_count = N_robot_joints;
		int rj_len = rj_count * 4;
		int packed_len = as_len + rj_len + 5; /* +n_guns, gun_num, state, as_len, rj_len */
		...
		/* The C caller expects a pointer into Robot_joints; the OCaml result
		   matches the same joints, so we can still use the original pointer. */
...
void set_robot_state(object* obj, int state)
{
...
#ifdef USE_OX_BRIDGE
	{
		...
		int rj_count = N_robot_joints;
		int rj_len = rj_count * 4;
		/* packed: anim_states + robot_joints + n_guns + anim_angles(30) + state + as_len + rj_len */
...
void robot_set_angles(robot_info* r, polymodel* pm, vms_angvec angs[N_ANIM_STATES][MAX_SUBMODELS])
{
...
#ifdef USE_OX_BRIDGE
	{
		...
		/* packed: n_guns(1) + gun_submodels(8) + n_models(1) + submodel_parents(10) +
		           angs(5*10*3=150) + joint_offset(1) = 171 */
		int32_t packed[171];
		int idx = 0;
		packed[idx++] = r->n_guns;
		...
		/* Max output: anim_states((n_guns+1)*5*2) + 1 + joints(n_models*5*4) */
		int32_t out_buf[2048];
		int out_len = 0;
		cd_ox_robot_set_angles(packed, 171, out_buf, &out_len);
		/* Unpack: anim_states_flat first */
		int n_guns = r->n_guns;
		int as_len = (n_guns + 1) * N_ANIM_STATES * 2;
		int oi = 0;
		for (int gg = 0; gg < n_guns + 1; gg++)
